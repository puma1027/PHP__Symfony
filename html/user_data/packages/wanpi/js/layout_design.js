...
// ===== 2013.06.06 RCHJ Add =====
var defHeaderInternalNavi  = 50;	// HeaderInternalNaviのデフォルトの高さ
var defBottomNavi   = 50;	// BottomNaviのデフォルトの高さ
// ========= End =======
...
function fnCreateArr( addEvt , top , left ){
	...
	// ==== RCHJ Add 2013.06.06 ====
	arrObjtmp['HeaderInternalNavi'] = Array();
	arrObjtmp['BottomNavi'] = Array();
	// ==== End ====
...
function fnSetTargetHeight(){
	...
	// === 2013.06.06 RCHJ Add =====
	var HeaderInternalNaviHeight = defHeaderInternalNavi; 
	var BottomNaviHeight = defBottomNavi;
	// ====== End ======
    ...
    for ( var cnt = 0; cnt < all_elms.length; cnt++ ) {
        ...
        if ( getAttrValue ( all_elms[cnt], 'class' ) == 'drop_target' ) {
			...
			// HeaderInternalNavi部領域の高さを保持(2013.06.06 RCHJ Add)
			if (tid == 'HeaderInternalNavi') {
				if (target_height > defHeaderInternalNavi) {
					HeaderInternalNaviHeight = target_height;
				}
			}	
			
			// BottomNavi部領域の高さを保持(2013.06.06 RCHJ Add)
			if (tid == 'BottomNavi') {
				if (target_height > defBottomNavi) {
					BottomNaviHeight = target_height;
				}
			}	
        }
	}
...
//	alert(NaviHeight+"/"+MainHeadHeight+"/"+MainFootHeight);
    ...
    for ( var cnt = 0; cnt < all_elms.length; cnt++ ) {
        ...
        if ( getAttrValue ( all_elms[cnt], 'class' ) == 'drop_target' ) {
			...
			// ======== 2013.06.06 RCHJ Add =========
			else if (tid == 'HeaderInternalNavi'){
				target_height = HeaderInternalNaviHeight;
			}else if (tid == 'BottomNavi'){
				target_height = BottomNaviHeight;
			}
			// ============== End ==============

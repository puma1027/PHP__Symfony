...
//                    }
//                    SC_Response_Ex::sendRedirect(URL_CART_TOP);
//                    SC_Response_Ex::actionExit();
//                }
//                break;
//
...
//        // ブランド情報の取得
//        $where = "brand_id = ?";
//        $ret = $objQuery->getrow("dtb_brand", "name, name_furigana, url, description", $where, array($this->arrProduct['brand_id']));
//        $this->arrBrand = $ret;
...
//            $ret = $objQuery->getrow("dtb_brand", "name, name_furigana, url, description", $where, array($this->arrProduct['haori_brand_id']));
//            $this->arrHaoriBrand = $ret;
//            $ret = $objQuery->getrow("dtb_brand", "name, name_furigana, url, description", $where, array($this->arrProduct['necklace_brand_id']));
//            $this->arrNecklaceBrand = $ret;
...
//                $ret = $objQuery->getrow("dtb_brand", "name, name_furigana, url, description", $where, array($this->arrProduct['bag_brand_id']));
//                $this->arrBagBrand = $ret;
//            }
//        }
//        // 商品コードの取得
...
// ========= 2012.07.03 RCHJ Change(dtb_products add) =============
        ...
        $from = "(dtb_review left join dtb_order_detail on dtb_order_detail.order_id = dtb_review.order_id) left join dtb_products on dtb_products.product_id = dtb_order_detail.product_id";
        $where = "dtb_review.del_flg = 0 AND dtb_review.status = 1 AND dtb_order_detail.product_id <> ?  AND (dtb_review.product_id = ? OR  dtb_review.product_list = '" . $id . "' OR  dtb_review.product_list like '%," . $id . ",%' OR  dtb_review.product_list like '" . $id . ",%' OR  dtb_review.product_list like '%," . $id . "' ) ORDER BY dtb_review.create_date DESC LIMIT " . REVIEW_REGIST_MAX;
// ============ end ============
        $arrval[] = $id;
        $arrval[] = $id;

 ...
 * @version $Id:LC_Page_Admin_Products_Inventory_Management_Necklace.php
 */
class LC_Page_Admin_Products_Inventory_Management_Necklace extends LC_Page_Admin_Ex {
    ...
    function init() {
        parent::init();
        $this->tpl_mainpage = 'extension/products/inventory_management_necklace.tpl';
        $this->tpl_mainno = 'products';
        $this->tpl_subno = 'inventory_management_necklace';
        $this->tpl_pager = SMARTY_TEMPLATES_REALDIR . 'admin/pager.tpl';  
        $this->tpl_subtitle = 'ネックレス在庫管理';   
...
//        SC_Utils_Ex::sfIsSuccess($objSess);
        ...
        // POSTデータを引き継ぐ
        $this->tpl_necklace_id = $_POST['necklace_id'];
        ...
        case 'edit':
            ...
            if(count($this->arrErr) <= 0) {
                ...
                $where = "product_code = ?";
                $objQuery->update("dtb_products_class", $sqlval, $where, array($_POST['product_code']));

                $this->tpl_necklace_id = null;
                // 再表示
//                $this->reload();
            } else {

                $where = "product_code = '".$_POST['necklace_id']."'";
                $setOrder = "B.product_code";
                $arrRet = $this->lfGetNecklaceDetail($where,$setOrder);
        ...
        // 削除
        case 'delete':
            // セットドレスのセット商品情報(ネックレス)を削除し、指定のネックレスをセット商品からはずす。
            $sqlval['set_pcode_necklace'] = "";
            $where = "product_code = ?";
        ...
        case 'add':
            ...
            $this->arrErr = $this->lfErrorCheck();
            if(count($this->arrErr) <= 0) {
                // セットドレスのセット商品情報(ネックレス)を追加する。
                $sql = "UPDATE dtb_products_class SET set_pcode_necklace = '".$_POST['product_code']."' WHERE product_code = '".$_POST['add_set_product']."';";
                $objQuery->query($sql);
            }
        ...
        // バッグの読込
        $where = "A.del_flg <> 1 AND (B.product_code LIKE '%".PCODE_NECKLACE_SMALL."%' OR B.product_code LIKE '%".PCODE_NECKLACE_LARGE."%')";
        $setOrder = "B.product_code";
        $this->arrNecklace = $this->lfGetNecklaceDetail($where,$setOrder);
    ...
    function lfErrorCheck() {
        ...
        if(!empty($_POST['add_set_product'])){
            if ((strpos($_POST['add_set_product'],PCODE_SET_DRESS ) !== false)) {
                $objQuery = SC_Query_Ex::getSingletonInstance();
                $arrRet = $objQuery->select("count(product_code)", "dtb_products_class", "set_pcode_necklace = '".$_POST['product_code']."' AND product_code = '".$_POST['add_set_product']."'");
                if ($arrRet[0]['count'] > 0 ) {
    ...
    // 
    function lfGetNecklaceDetail($where,$setOrder){

        $this_week = $this->get_week(date("Y"),date("m"),date("d"));

        $objQuery = SC_Query_Ex::getSingletonInstance();
        $objQuery->setOrder($setOrder);
        $arrRet = $objQuery->select("A.product_id,B.product_code,main_image,stock", "(dtb_products AS A LEFT JOIN dtb_products_class AS B ON A.product_id = B.product_id)", $where);


        foreach ($arrRet as $key => $val) {
            //セットドレスの発送数を数える------------------------------------------------------------------------
            $where = "A.del_flg <> 1 AND A.status <> 2 AND set_pcode_necklace LIKE '%".substr($arrRet[$key]['product_code'], 0, 7)."%' AND D.sending_date BETWEEN '".$this_week[0]."' AND '".$this_week[1]."'";
            $objQuery->setOrder("set_pcode_necklace, B.product_code");
            $arrRetEx = $objQuery->select("sending_date,B.product_code", "((dtb_products AS A LEFT JOIN dtb_products_class AS B ON A.product_id = B.product_id)  LEFT JOIN dtb_order_detail C ON A.product_id = C.product_id)  LEFT JOIN dtb_order AS D ON D.order_id = C.order_id ", $where);

            $temp='';
            $count_dress=0;
            foreach ($arrRetEx as $keyEx=>$valEx) {
                if ((strpos($arrRetEx[$keyEx]['product_code'],PCODE_NECKLACE_SMALL) === false) || (strpos($arrRetEx[$keyEx]['product_code'],PCODE_NECKLACE_LARGE) === false)) {
            ...
            $temp='';
            $chancel_pcode_necklace=array();
            $count_necklace=0;
            $count_chancel_necklace=0;
            foreach ($arrRetSingleEx as $keyEx=>$valEx) {
                if ((strpos($arrRetEx[$keyEx]['product_code'],PCODE_NECKLACE_SMALL) !== false) || (strpos($arrRetEx[$keyEx]['product_code'],PCODE_NECKLACE_LARGE) !== false)) {
                    if ($arrRetSingleEx[$keyEx]['reserved_type'] != RESERVE_PATTEN_SPECDAY) {
                        $temp[$count_necklace] = $arrRetSingleEx[$keyEx]['product_code'];
                        $count_necklace++;
                    } else {
                        if (!in_array($arrRetSingleEx[$keyEx]['product_code'], $chancel_pcode_necklace)) {
                            $chancel_pcode_necklace[$count_chancel_necklace] = $arrRetSingleEx[$keyEx]['product_code'];
                            $count_chancel_necklace++;
                        }
                    }
                }
            }
            $arrRet[$key]['shipping_product_single'] = $temp;
            $arrRet[$key]['shipping_this_week_single'] = $count_necklace;
            $arrRet[$key]['shipping_this_week'] = $arrRet[$key]['shipping_this_week'] + $count_necklace;
            $arrRet[$key]['shipping_chancel'] = $count_chancel_necklace;
            $arrRet[$key]['shipping_chancel_product_set'] = $chancel_pcode_necklace;

            //使用しているセット商品を求める------------------------------------------------------------------------
            $where = "A.del_flg <> 1 AND A.status <> 2 AND set_pcode_necklace LIKE '%".substr($arrRet[$key]['product_code'], 0, 7)."%'";

 ...
 * @version $Id:LC_Page_Admin_Products_Inventory_Management_Stole.php
 */
class LC_Page_Admin_Products_Inventory_Management_Stole extends LC_Page_Admin_Ex {
    ...
    function init() {
        parent::init();
        $this->tpl_mainpage = 'extension/products/inventory_management_stole.tpl';
        $this->tpl_mainno = 'products';
        $this->tpl_subno = 'inventory_management_stole';
        $this->tpl_pager = SMARTY_TEMPLATES_REALDIR . 'admin/pager.tpl'; 
        $this->tpl_subtitle = 'ストール在庫管理';
...
//        SC_Utils_Ex::sfIsSuccess($objSess);
        ...
        // POSTデータを引き継ぐ
        $this->tpl_stole_id = $_POST['stole_id'];
        ...
        case 'edit':
            ...
            if(count($this->arrErr) <= 0) {
                ...
                $this->tpl_stole_id = null;
                // 再表示
//                $this->reload();

            } else {

                $where = "product_code = '".$_POST['stole_id']."'";
                $setOrder = "B.product_code";
                $arrRet = $this->lfGetStoleDetail($where,$setOrder);
        ...
        case 'add':
            ...
            if(count($this->arrErr) <= 0) {
                ...
                $sql = "UPDATE dtb_products_class SET set_pcode_stole = '".$_POST['product_code']."' WHERE product_code = '".$_POST['add_set_product']."';";
                $objQuery->query($sql);
            }
        ...
        // バッグの読込
        $where = "A.del_flg <> 1 AND B.product_code LIKE '%".PCODE_STOLE."%'";
        $setOrder = "B.product_code";
        $this->arrStole = $this->lfGetStoleDetail($where,$setOrder);
    ...
    function lfErrorCheck() {
        ...
        if(!empty($_POST['add_set_product'])){
            if ((strpos($_POST['add_set_product'],PCODE_SET_DRESS ) !== false)) {
                $objQuery = SC_Query_Ex::getSingletonInstance();
                $arrRet = $objQuery->select("count(product_code)", "dtb_products_class", "set_pcode_stole = '".$_POST['product_code']."' AND product_code = '".$_POST['add_set_product']."'");
                if ($arrRet[0]['count'] > 0 ) {
    ...
    //
    function lfGetStoleDetail($where,$setOrder){
        ...
        foreach ($arrRet as $key => $val) {
            //セットドレスの発送数を数える------------------------------------------------------------------------
            $where = "A.del_flg <> 1 AND A.status <> 2 AND set_pcode_stole LIKE '%".substr($arrRet[$key]['product_code'], 0, 7)."%' AND C.sending_date BETWEEN '".$this_week[0]."' AND '".$this_week[1]."'";
            $objQuery->setOrder("set_pcode_stole, B.product_code");
            ...
            foreach ($arrRetEx as $keyEx=>$valEx) {
                if (strpos($arrRetEx[$keyEx]['product_code'],PCODE_STOLE) === false) {
            ...
            $temp='';
            $chancel_pcode_stole=array();
            $count_stole=0;
            $count_chancel_stole=0;
            foreach ($arrRetSingleEx as $keyEx=>$valEx) {
                if (strpos($arrRetSingleEx[$keyEx]['product_code'],PCODE_STOLE) !== false) {
                    if ($arrRetSingleEx[$keyEx]['reserved_type'] != RESERVE_PATTEN_SPECDAY) {
                        $temp[$count_stole] = $arrRetSingleEx[$keyEx]['product_code'];
                        $count_stole++;
                    } else {
                        if (!in_array($arrRetSingleEx[$keyEx]['product_code'], $chancel_pcode_stole)) {
                            $chancel_pcode_stole[$count_chancel_stole] = $arrRetSingleEx[$keyEx]['product_code'];
                            $count_chancel_stole++;
                        }
                    }
                }
            ...
            $arrRet[$key]['shipping_this_week_single'] = $count_stole;
            $arrRet[$key]['shipping_this_week'] = $arrRet[$key]['shipping_this_week'] + $count_stole;
            $arrRet[$key]['shipping_chancel'] = $count_chancel_stole;
            $arrRet[$key]['shipping_chancel_product_set'] = $chancel_pcode_stole;


            //使用しているセット商品を求める------------------------------------------------------------------------
            $where = "A.del_flg <> 1 AND A.status <> 2 AND set_pcode_stole LIKE '%".substr($arrRet[$key]['product_code'], 0, 7)."%'";

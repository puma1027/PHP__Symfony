...
class LC_Page_Admin_Products_Product extends LC_Page_Admin_Products_Ex
{
    ...
    public function init()
    {
        ...
        $this->arrICON = $masterData->getMasterData('mtb_icon');  // ADD KGS_20140313
        $this->arrFIGUREDETAIL = $masterData->getMasterData("mtb_figure_detail");   // ADD KGS_20140313
    ...
    public function action()
    {
        ...
        switch ($mode) {
            ...
            case 'edit':  
                ...
                //if ( !isset( $arrForm['product_flag'] ) ) $arrForm['product_flag'] = "";
                //$arrForm['product_flag'] = SC_Utils_Ex::sfMergeCheckBoxes($arrForm['product_flag'], count($this->arrSTATUS));
                // 20210201 add ishibashi
                $array['product_flag'] = SC_Product::convertProductFlag($array['product_flag']);
                //R.K　2012/02/14　追加開始
                // 商品アイコン項目の変換
                if ( !isset( $arrForm['icon_flag'] ) ) $arrForm['icon_flag'] = "";
                $arrForm['icon_flag'] = SC_Utils_Ex::sfMergeCheckBoxes($arrForm['icon_flag'], count($this->arrICON));
            ...
            case 'confirm_return':
                ...
                $this->lfInitFormParam($objFormParam, $_POST);
                $arrForm = $objFormParam->getHashArray();
                //if(isset($arrForm['product_flag']) && !is_array($arrForm['product_flag'])) {
                //    // 商品ステータスの分割読込
                //    $arrForm['product_flag'] = SC_Utils_Ex::sfSplitCheckBoxes($arrForm['product_flag']);
                //}
                // 20210201 add ishibashi
                $this->arrForm['product_flag'] = SC_Product::getProductFlag($this->arrForm['product_flag']);
                //R.K　2012/02/14　追加開始
                // 商品アイコン項目の変換
                if(isset($arrForm['icon_flag']) && !is_array($arrForm['icon_flag'])) {
                    // 商品ステータスの分割読込
                    $arrForm['icon_flag'] = SC_Utils_Ex::sfSplitCheckBoxes($arrForm['icon_flag']);
                }
                // 入力画面表示設定
                $this->arrForm = $this->lfSetViewParam_InputPage($objUpFile, $objDownFile, $arrForm);
                // ページonload時のJavaScript設定
        ...
        if($_POST['mode'] != 'pre_edit') {
            ...
            $this->arrCoordinateRecommend = $this->lfGetRecommendProducts($this->arrForm,"coordinate_", COORDINATE_RECOMMEND_PRODUCT_MAX);
            $this->arrSizeColorRecommend = $this->lfGetRecommendProducts($this->arrForm,"size_color_", SIZE_COLOR_RECOMMEND_PRODUCT_MAX);
            // ================ end ===============
        }
    ...
    public function lfInitFormParam(&$objFormParam, $arrPost)
    {
        ...
        $objFormParam->addParam('商品ステータス', 'product_flag', LTEXT_LEN, "KVa", array("SPTAB_CHECK","MAX_LENGTH_CHECK"));
        
        $objFormParam->addParam('アイコン', 'icon_flag', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));  // ADD KGS_20140313
...
//{{add bhm_20140320
        for ($cnt = 1; $cnt <= 11; $cnt++) { 
            $objFormParam->addParam('フォトギャラリーコメン' . $cnt, 'photo_gallery_comment' . $cnt, STEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
            $objFormParam->addParam('フォトギャラリー画像' . $cnt, 'photo_gallery_image' . $cnt, '', '', array());      
        }
        
        for ($cnt = PHOTO_GALLERY_IMAGE_NUM-2; $cnt <= PHOTO_GALLERY_IMAGE_NUM; $cnt++) { 
            $objFormParam->addParam('詳細-サブコメント' . $cnt, 'photo_gallery_comment' . $cnt, STEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
            $objFormParam->addParam('詳細-サブ画像' . $cnt, 'photo_gallery_image' . $cnt, '', '', array());      
        }
//}}add bhm_20140320

        for ($cnt = 1; $cnt <= COORDINATE_RECOMMEND_PRODUCT_MAX; $cnt++) {
            $objFormParam->addParam('関連商品コメント' . $cnt, 'coordinate_recommend_id' . $cnt, LTEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
            $objFormParam->addParam('関連商品ID' . $cnt, 'coordinate_recommend_id' . $cnt, INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
            $objFormParam->addParam('recommend_delete' . $cnt, 'coordinate_recommend_delete' . $cnt, '', 'n', array());
        }
        for ($cnt = 1; $cnt <= SIZE_COLOR_RECOMMEND_PRODUCT_MAX; $cnt++) {
            $objFormParam->addParam('関連商品コメント' . $cnt, 'size_color_recommend_comment' . $cnt, LTEXT_LEN, 'KVa', array('SPTAB_CHECK', 'MAX_LENGTH_CHECK'));
            $objFormParam->addParam('関連商品ID' . $cnt, 'size_color_recommend_id' . $cnt, INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'));
            $objFormParam->addParam('recommend_delete' . $cnt, 'size_color_recommend_delete' . $cnt, '', 'n', array());
        }
    ...
    {
        $objUpFile->addFile('一覧-メイン画像', 'main_list_image', array('jpg', 'gif', 'png', 'webp'), IMAGE_SIZE, false, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT);
        $objUpFile->addFile('詳細-メイン画像', 'main_image', array('jpg', 'gif', 'png', 'webp'), IMAGE_SIZE, false, NORMAL_IMAGE_WIDTH, NORMAL_IMAGE_HEIGHT);
        $objUpFile->addFile('詳細-メイン拡大画像', 'main_large_image', array('jpg', 'gif', 'png', 'webp'), IMAGE_SIZE, false, LARGE_IMAGE_WIDTH, LARGE_IMAGE_HEIGHT);
        for ($cnt = 1; $cnt <= PRODUCTSUB_MAX; $cnt++) {
            $objUpFile->addFile("詳細-サブ画像$cnt", "sub_image$cnt", array('jpg', 'gif', 'png', 'webp'), IMAGE_SIZE, false, NORMAL_SUBIMAGE_WIDTH, NORMAL_SUBIMAGE_HEIGHT);
            $objUpFile->addFile("詳細-サブ拡大画像$cnt", "sub_large_image$cnt", array('jpg', 'gif', 'png', 'webp'), IMAGE_SIZE, false, LARGE_SUBIMAGE_WIDTH, LARGE_SUBIMAGE_HEIGHT);
        }
    }
...
__EOF__;
        ...
        $arrProduct[0]['product_flag'] = $productStatus[$product_id];
     ...
     * @return array  関連商品データ配列
     */
    public function lfGetRecommendProducts(&$arrForm, $pre_name="", $count=RECOMMEND_PRODUCT_MAX)
    {
        $arrRecommend = array();
...
//            $commentkey = 'recommend_comment' . $i;
        for ($i = 1; $i <= $count; $i++) {
            ...
            if ((isset($arrForm[$keyname]) && !empty($arrForm[$keyname])) && $arrForm[$delkey] != 1) {
                $arrRet = $objQuery->select("main_list_image, product_code_min, name", "vw_products_allclass AS allcls", "product_id = ?", array($arrForm[$keyname]));
                //$objProduct = new SC_Product_Ex(); //del BHM_20140319
                //$objProduct->getDetail($arrForm[$keyname]); //del BHM_20140319
    ...
    public function lfRegistProduct(&$objUpFile, &$objDownFile, $arrList)
    {
        ...
        // 配列の添字を定義
        /* chages bhm_20140319
        $checkArray = array('name', 'status', "icon_flag",'product_flag',
        ...
        */                            
        ...
        $objProduct->setProductStatus($product_id, $arrList['product_flag']);
     ...
     * @return void
     */
    public function lfInsertRecommendProducts(&$objQuery, $arrList, $product_id, /*change bhm_20140319*/$del_flag = false, $pre_name="", $status = "0", $count=RECOMMEND_PRODUCT_MAX)
    {
        // 一旦関連商品を全て削除する
        if($del_flag){       //change bhm_20140319
            $objQuery->delete('dtb_recommend_products', 'product_id = ?', array($product_id));
        }                    //change bhm_20140319
        $sqlval['product_id'] = $product_id;
        //$rank = RECOMMEND_PRODUCT_MAX; //change bhm_20140319
        $rank = $count;
        for($i = 1; $i <= $count; $i++) {
            $keyname = $pre_name . "recommend_id" . $i;
            $commentkey = $pre_name . "recommend_comment" . $i;
            ...
            if ($arrList[$keyname] != '' && $arrList[$deletekey] != '1') {
                ...
                $sqlval['create_date'] = 'CURRENT_TIMESTAMP';
                $sqlval['update_date'] = 'CURRENT_TIMESTAMP';
                $sqlval['status'] = $status; //add bhm_20140319
    ...
    public function hasSubProductData($arrSubProductData)
    {
        ...
        for ($i = PHOTO_GALLERY_IMAGE_NUM - 2; $i <= PHOTO_GALLERY_IMAGE_NUM; $i++) {
            if (SC_Utils_Ex::isBlank($_POST['photo_gallery_comment'.$i]) == false
               || SC_Utils_Ex::isBlank($_POST['photo_gallery_image'.$i]) == false        
            ) {

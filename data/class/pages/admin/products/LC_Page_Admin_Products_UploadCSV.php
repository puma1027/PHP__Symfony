...
class LC_Page_Admin_Products_UploadCSV extends LC_Page_Admin_Ex
{
    ...
    public function doUploadCsv(&$objFormParam, &$objUpFile)
    {
        ...
        system('sudo chmod 777'. $enc_filepath);exit;
        // CSVファイルのオープン
...
//        $objFormParam->addParam("規格分類ID", "classcategory_id",INT_LEN, "n", array("NUM_CHECK","MAX_LENGTH_CHECK"));   //classcategory_id1      規格名1
//        $objFormParam->addParam("親規格分類ID", "parent_classcategory_id",INT_LEN, "n", array("NUM_CHECK","MAX_LENGTH_CHECK"));  // classcategory_id2   規格名2
//       
//        $objFormParam->addParam("規格分類名", "classcategory_name", STEXT_LEN, "KVa", array("SPTAB_CHECK","MAX_LENGTH_CHECK"));
//        $objFormParam->addParam("親規格分類名", "parent_classcategory_name", STEXT_LEN, "KVa", array("SPTAB_CHECK","MAX_LENGTH_CHECK"));
//        
//        $objFormParam->addParam("メーカーID", "maker_id", INT_LEN, "n", array("NUM_CHECK","MAX_LENGTH_CHECK"));
//        $objFormParam->addParam("メーカー名", "maker_name", STEXT_LEN, "KVa",array("SPTAB_CHECK","MAX_LENGTH_CHECK"));   

//        $objFormParam->addParam("商品名", "name", STEXT_LEN, "KVa", array("EXIST_CHECK","SPTAB_CHECK","MAX_LENGTH_CHECK"));
//        $objFormParam->addParam("表示ステータス(公開・非公開)", "status", INT_LEN, "n", array("EXIST_CHECK","MAX_LENGTH_CHECK","NUM_CHECK"));
//        
//        $objFormParam->addParam("メーカーURL(コメント1)", "comment1", URL_LEN, "KVa", array("SPTAB_CHECK","MAX_LENGTH_CHECK","URL_CHECK")); 
//        
//            $objFormParam->addParam("コメント2", "comment2", LLTEXT_LEN, "KVa", array("SPTAB_CHECK","MAX_LENGTH_CHECK"));
//         
//        $objFormParam->addParam("検索ワード(コメント3)", "comment3", LLTEXT_LEN, "KVa", array("SPTAB_CHECK","MAX_LENGTH_CHECK")); 
//        
//            $objFormParam->addParam("コメント4", "comment4", LLTEXT_LEN, "KVa", array("SPTAB_CHECK","MAX_LENGTH_CHECK"));
//            $objFormParam->addParam("コメント5", "comment5", LLTEXT_LEN, "KVa", array("SPTAB_CHECK","MAX_LENGTH_CHECK"));
//            $objFormParam->addParam("コメント6", "comment6", LLTEXT_LEN, "KVa", array("SPTAB_CHECK","MAX_LENGTH_CHECK"));
//        
//        
...
//        $objFormParam->addParam("在庫数", "stock", INT_LEN, "n", array("MAX_LENGTH_CHECK","NUM_CHECK"));  //change
...
//        $objFormParam->addParam("商品カテゴリ", "category_ids", STEXT_LEN, "n", array("EXIST_CHECK", "SPTAB_CHECK"));
//        $objFormParam->addParam("カテゴリ名", "category_names", LTEXT_LEN, "KVa", array("SPTAB_CHECK","MAX_LENGTH_CHECK"));
//        
//        $objFormParam->addParam("ダウンロード実ファイル", "down_realfilename", STEXT_LEN, "n", array("EXIST_CHECK", "SPTAB_CHECK")); 
//        $objFormParam->addParam("ダウンロード実ファイル", "down_filename", STEXT_LEN, "n", array("EXIST_CHECK", "SPTAB_CHECK")); 

//        $objFormParam->addParam("在庫無制限フラグ", "stock_unlimited", INT_LEN, "n", array("NUM_CHECK","MAX_LENGTH_CHECK")); 
//        $objFormParam->addParam("商品種別ID", "product_type_id", INT_LEN, "n", array("NUM_CHECK","MAX_LENGTH_CHECK")); 
    }
...
//            $objProduct->setProductStatus($product_id, $arrStatus_id);
        if (isset($arrList['product_flag'])) { 
            $arrStatus_id = explode(',', $arrList['product_flag']);   
            $objProduct->setProductStatus($product_id, $arrStatus_id);  
        }
    ...
    public function lfRegistProductClass($objQuery, $arrList, $product_id, $product_class_id)
    {
        ...
        if ($product_class_id == '') {
            ...
            $objQuery->insert1('dtb_products_class', $sqlval);   

...
class LC_Page_Admin_Total extends LC_Page_Admin_Ex
{
    ...
    public function init()
    {
		...
		$this->tpl_subno = 'index'; // Add KH_2014_3_7
		...
		$this->arrTitle['order_analysis'] = "受注分析";   // Add KH_2014_3_7
        $this->arrTitle['product_analysis'] = "商品分析";// Add KH_2014_3_7
...
var_dump(ORDER_CANCEL);
    ...
    public function lfGetOrderTerm($type, $sdate, $edate)
    {
        ...
        //::$where = " WHERE T1.del_flg = 0 AND T1.status <> " . ORDER_CANCEL . " AND T1.create_date BETWEEN '$start' AND '$end' ";//::N00055 Del 20130523
        $where = " WHERE T1.del_flg = 0 AND T1.status <> " . ORDER_STATUS_CANCEL . " AND T1.sending_date BETWEEN '$sdate' AND '$edate' ";//::N00055 Add 20130523

			$tbl_name = "(SELECT sending_date, COUNT(order_id) as total_order, 0 AS men, 0 AS women, 0 AS men_member, 0 AS men_nonmember, 0 AS women_member, 0 AS women_nonmember, SUM(payment_total) AS total, AVG(total) AS total_average  $from $where GROUP BY sending_date 
			UNION 
			SELECT sending_date, 0 as total_order, COUNT(order_id) AS men, 0 as women, 0 AS men_member, 0 AS men_nonmember, 0 AS women_member, 0 AS women_nonmember, 0 AS total, 0 AS total_average  $from $where AND T1.order_sex= 1  GROUP BY sending_date 
			UNION 
			SELECT sending_date, 0 as total_order, 0 AS men, COUNT(order_id) as women, 0 AS men_member, 0 AS men_nonmember, 0 AS women_member, 0 AS women_nonmember, 0 AS total, 0 AS total_average  $from $where AND T1 .order_sex= 2  GROUP BY sending_date 
			UNION
			SELECT sending_date, 0 as total_order, 0 AS men, 0 as women, COUNT(order_id) AS men_member, 0 AS men_nonmember, 0 AS women_member, 0 AS women_nonmember, 0 AS total, 0 AS total_average $from $where AND T1.order_sex = 1 AND T2.customer_id <> 0  GROUP BY sending_date 
			UNION
			SELECT sending_date, 0 as total_order, 0 AS men, 0 as women, 0 AS men_member, COUNT(order_id) AS men_nonmember, 0 AS women_member, 0 AS women_nonmember, 0 AS total, 0 AS total_average  $from $where AND T1.order_sex = 1 AND T2.customer_id = 0  GROUP BY sending_date 
			UNION
			SELECT sending_date, 0 as total_order, 0 AS men, 0 as women, 0 AS men_member, 0 AS men_nonmember, COUNT(order_id) AS women_member, 0 AS women_nonmember, 0 AS total, 0 AS total_average $from $where AND T1.order_sex = 2 AND T2.customer_id <> 0  GROUP BY sending_date 
			UNION
			SELECT sending_date, 0 as total_order, 0 AS men, 0 as women, 0 AS men_member, 0 AS men_nonmember, 0 AS women_member, COUNT(order_id) AS women_nonmember, 0 AS total, 0 AS total_average  $from $where  AND T1.order_sex = 2 AND T2.customer_id = 0 GROUP BY sending_date ) tmp";

		$arrTotalResults = $objQuery->select($col, $tbl_name);

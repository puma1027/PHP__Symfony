...
require_once CLASS_REALDIR . "SC_Order_Pdf.php";
...
class LC_Page_Admin_Order_Pdf extends LC_Page_Admin_Ex
{
    ...
    if($arrRet['title'] == '') $arrRet['title'] = 'お買上げ明細書(納品書)';
        ...
        // エラー入力なし
        if (count($arrErr) == 0) {
            //$objFpdf = new SC_Fpdf_Ex($arrRet['download'], $arrRet['title'],'template_nouhin02.pdf');
            $objFpdf = new SC_Order_Pdf($arrRet['download'] ,'template_nouhin02.pdf');
            foreach ($arrRet['order_id'] AS $key => $val) {
                $arrPdfData = $arrRet;
                $arrPdfData['order_id'] = $val;
                $objFpdf->setData($arrPdfData,$arrRet['title']);
            }
            $objFpdf->createPdf();

...
class LC_Page_Admin_Order_Edit extends LC_Page_Admin_Order_Ex
{
    ...
    public function action()
    {
        ...
        switch ($this->getMode()) {
            ...
            case 'edit':
                ...
                if (SC_Utils_Ex::isBlank($this->arrErr)) {
                    $message = '受注を編集しました。\n\n【発送日】まだ先であることを確認！\n【お届け時間】指定できる地域か確認！\n【お届け先】「お届け先情報」欄を変更したか確認！\n【お支払い方法】「超速便」地域ではないか確認！';
                    $order_id = $this->doRegister($order_id, $objPurchase, $objFormParam, $message, $arrValuesBefore);
                    if ($order_id >= 0) {
                        $this->setOrderToFormParam($objFormParam, $order_id);
                    }else{
						$message = "'受注履歴の編集が失敗しました。すでに予約された商品が、入力されていないか確認してください。'";
					}
					...
					if($ordrer_status == ORDER_STATUS_CANCEL){

						$mailHelper = new SC_Helper_Mail_Ex();

						$mailHelper->sfSendOrderMail($order_id, '3');
					}
                }
                break;
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                    ...
                    if (SC_Utils_Ex::isBlank($this->arrErr)) {
                        ...
                        if ($order_id >= 0) {
							...
							$message = '新規受注の登録に失敗しました。すでに予約された商品が、入力されていないか確認してください。';
						}
                        $this->tpl_onload = "window.alert('" . $message . "');";
                    }
        ...
        // 2020.09.11 hori 単品のドレスの場合税率10%で計算
        $max = count($this->arrForm['quantity']['value']);
        for ($i = 0; $i < $max; $i++) {
            if ($this->arrForm['price']['value'][$i] === '5436') {
                $this->arrForm['tax_rate']['value'][$i] = '10';
            }
        }
        ...
        $org_deliv_id = $objFormParam->getValue('deliv_id');
        if ($org_deliv_id == "0") {
            $org_deliv_id  = "2";
        }
        //$this->arrDelivTime   = SC_Helper_Delivery_Ex::getDelivTime($org_deliv_id);

        // 202007 hori 県によって時間の選択肢変更
        $deliv_pref = $objFormParam->getFormParamList()['shipping_pref']['value'][0];
        $delivTimeList = SC_Helper_Delivery_Ex::getDelivTime($org_deliv_id);
        $this->arrDelivTime = SC_Helper_Delivery_Ex::sfGetSummaryDelivTime($deliv_pref, array_values($delivTimeList));
        $this->arrDelivTime = array_intersect($delivTimeList, $this->arrDelivTime);
        // 都道府県がないときはすべての時間帯を表示
        if (empty($this->arrDelivTime)) $this->arrDelivTime = $delivTimeList;
    ...
    public function lfInitParam(&$objFormParam)
    {
        ...
        $objFormParam->addParam('あんしん保証', 'relief_value', INT_LEN, 'n', array('EXIST_CHECK', 'MAX_LENGTH_CHECK', 'NUM_CHECK'), '0');
        ...
        $objFormParam->addParam("単価", "price", INT_LEN, "n", array("EXIST_CHECK", "MAX_LENGTH_CHECK", "NUM_POINT_CHECK"), '0');//::KHS Change 2014.3.14
        ...
        $objFormParam->addParam('追加変更日', 'update_date');
        ...
        //KHS Change 2014.3.14
        $objFormParam->addParam("発送日指定", "sending_date", STEXT_LEN, "KVa", array("EXIST_CHECK", "MAX_LENGTH_CHECK"));
        ...
        //KHS ADD 2014.3.14
        $objFormParam->addParam("同梱注文番号", "include_orderid");
        $objFormParam->addParam("セット商品コード", "set_pid");//::N00083
        $objFormParam->addParam("セット商品タイプ", "set_ptype");//::N00083
    }
    ...
    public function lfCheckError(&$objFormParam)
    {
        ...
        */
        ...
        $totaltax = 0;
        for ($i = 0; $i < $max; $i++) {
            // 2020.09.11 hori 単品ドレスの場合だけ税率10%での計算
            if ($arrValues['price'][$i] === '5436') {
                $arrValues['tax_rate'][$i] = '10';
            }
            // 小計の計算
        ...
        // 合計
        $arrValues['total'] = $subtotal - $arrValues['discount'] + $arrValues['deliv_fee'] + $arrValues['charge'] + $arrValues['relief_value'];
        // お支払い合計
...
//============= KHS Add 2014.3.16==========
       ...
       $this->arrErr = $this->lfMoreCheek2($objFormParam);
        ...
        $arr_rental_period = $objReserveUtil->getRentalDay($arrRet['sending_date'][0]);
        ...
        $sqlval_reserve['customer_id'] = empty($arrRet['customer_id'])?'0':$arrRet['customer_id'];//$arrRet['customer_id'];
        $sqlval_reserve['order_id'] = $order_id;
        $sqlval_reserve['sending_date'] = $arrRet['sending_date'][0];

        $send_date_time = strtotime($arrRet['sending_date'][0]);
        ...
        // 2020.08.25 hori 予約キャンセルの処理の場合はdtb_products_reservedを削除したままにする
        // dtb_products_reservedが削除されてないとサイト側のカレンダーに反映されない
        if( $arrValues['status'] !== '8' ) {
            for($i = 0; $i < $max; $i++) {
        ...
        // deliv_date
        $arrValues["deliv_date"] = $arr_rental_period["arrival_day"];

// =================== KHS end 2014.3.16==============


//::B00120 Add 20140625
        $arrSendingDate = $objFormParam->getSwapArray(array("sending_date"));
        $objReserveUtil = new SC_Reserve_Utils();
        $arr_rental_period = $objReserveUtil->getRentalDay($arrSendingDate[0]["sending_date"]);
//::B00120 end 20140625
        ...
        //KHS ADD 2014.3.24
        $set_dress_product_id = null;
        $bag_is_on_flag = FALSE;
        for($i = 0; $i < $max; $i++) {
            //ドレスがあった場合は、セット商品としてのヒモ付けが行われた場合のために、product_idを保持しておく
            if (strpos($arrDetail[$i]['product_code'],PCODE_SET_DRESS) !== false || strpos($arrDetail[$i]['product_code'], PCODE_KIDS ) !== false ) {
                $set_dress_product_id = $arrDetail[$i]['product_id'];
            }
            //バッグがあった場合は、ドレスのproduct_typeを4点セットにしたいので、保持しておく
            if (strpos($arrDetail[$i]['product_code'],PCODE_BAG) !== false) {
                $bag_is_on_flag = TRUE;
            }
        }

         for($i = 0; $i < $max; $i++) {
           //セット商品としてのヒモ付け指定が行われているか検査する
              if ($_POST['set_check'][$i] === "1") {
                $arrDetail[$i]['set_pid'] = $set_dress_product_id;
                switch (substr($arrDetail[$i]['product_code'],0,3)) {
                case PCODE_ONEPIECE_ALL:
                case PCODE_ONEPIECE_SUMMER:
                case PCODE_ONEPIECE_WINTER:
                    $arrDetail[$i]['set_ptype'] = ONEPIECE_PRODUCT_TYPE;
                    break;
                case PCODE_DRESS:
                    $arrDetail[$i]['set_ptype'] = DRESS_PRODUCT_TYPE;
                    break;
                case PCODE_SET_DRESS:
                    //バッグがある場合はバッグ有り(4点)セットにする
                    if ($bag_is_on_flag) {
                        $arrDetail[$i]['set_ptype'] = 4;
                    } else {
                        $arrDetail[$i]['set_ptype'] = SET_DRESS_PRODUCT_TYPE;
                    }
                    break;
                case PCODE_STOLE:
                case PCODE_PANNIER:
                    $arrDetail[$i]['set_ptype'] = STOLE_PRODUCT_TYPE;
                    break;
                case PCODE_NECKLACE_SMALL:
                case PCODE_NECKLACE_LARGE:
                    $arrDetail[$i]['set_ptype'] = NECKLACE_PRODUCT_TYPE;
                    break;
                case PCODE_BELT:
                case PCODE_CORSAGE:
                case PCODE_BRACELET:
                case PCODE_BROOCH:
                case PCODE_BAG:
                    $arrDetail[$i]['set_ptype'] = OTHERS_PRODUCT_TYPE;
                    break;
                default:
                    break;
                }
            }else{
                $arrDetail[$i]['set_pid'] =null;
                $arrDetail[$i]['set_ptype'] =null;
            }
         }
        //KHS END
        ...
        $arrDelivTime = SC_Helper_Delivery_Ex::getDelivTime($org_deliv_id);
        //商品単価を複数配送にも適応
...
//                                              $arrShipping['shipping_date_day']);
            ...
            //KHS add 2014.3.14
            //::$arrShippingValues[$shipping_index]['shipping_date'] = $arrShipping['sending_date'];//::B00120 Del 20140625
            $arrShippingValues[$shipping_index]['shipping_date'] = $arr_rental_period["arrival_day"];//::B00120 Add 20140625
...
//KHS ADD   2014.3.25
            ...
            $arrRet = $objDb->sfGetProductsClass(array($arrAddProductInfo['product_id'], '', ''));
            $arrAddProductInfo['point_rate'] = $arrRet['point_rate'];
//KHS END
            foreach ($this->arrProductKeys as $insert_key) {
                $value = $objFormParam->getValue($insert_key);
                $arrAddProducts[$insert_key]   = (is_array($value))? $value: array();
                /** 2014.08.07 B.C. EDIT START */
                $add_product_class_id = $objFormParam->getValue('add_product_class_id');
                if (!empty($add_product_class_id)){
                    $arrAddProducts[$insert_key][] = $arrAddProductInfo[$insert_key];
                }else{
            		$arrAddProducts[$insert_key][$no] = $arrAddProductInfo[$insert_key];
                }
    	        /** 2014.08.07 B.C. EDIT END */
            }
    ...
    function lfMoreCheek2(&$objFormParam, $arr_product = array()){
      ...
      for($i = 0; $i < $max; $i++) {
        ...
        $count = $objQuery->count("dtb_products_reserved", $where, array($arr_product[$i],$arrVal['sending_date'][0], $arrVal['sending_date'][0], $arrVal['sending_date'][0]));
        //::N00083 Add 20131201
        $where = "product_id = ?";
        $arrRet = $objQuery->select("stock", "dtb_products_class", $where, array($arr_product[$i]));
        //::N00083 end 20131201
        //if($count > 0 || empty($arrVal['sending_date'])){
        if(($count >= $arrRet[0]['stock']) || empty($arrVal['sending_date'])) {//::N00083 Change 20131201

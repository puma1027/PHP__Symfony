 ...
 * @version $Id$
 */
class LC_Page_Mypage_LoginCheck extends LC_Page_AbstractMypage_Ex {
       ...
       switch($_POST['mode']) {
        case 'login':
            ...
            if($objCustomer->getCustomerDataFromEmailPass($arrForm['mypage_login_pass'], $arrForm['mypage_login_email'], true)) {
				...
				if($arrForm['mypage_login_memory'] == "1"){
					$objCookie->setCookie('login_pass', $arrForm['mypage_login_pass']);
				}
				
				if($_REQUEST['direct'] == 'favorite'){
					SC_Response_Ex::sendRedirect($this->getLocation("./favorite.php"));
    ...
    function lfInitParam() {
        ...
        $this->objFormParam->addParam("パスワード", "mypage_login_pass", PASSWORD_MIN_LEN, "KVa", array("EXIST_CHECK"));
        $this->objFormParam->addParam("パスワード", "mypage_login_pass1", PASSWORD_MIN_LEN, "KVa", array("EXIST_CHECK", "MIN_LENGTH_CHECK"));
        $this->objFormParam->addParam("パスワード", "mypage_login_pass2", PASSWORD_MAX_LEN, "KVa", array("EXIST_CHECK", "MAX_LENGTH_CHECK"));
    }

...
class LC_Page
{
    ...
    public function init()
    {
        ...
        $sql = "SELECT id, name,roma_text FROM mtb_pref";
        ...
        for($num=0; $num < 47; $num++){
            if(strpos($_SERVER['PHP_SELF'],$arrRes[$num]['roma_text']) !== false){
                $this->text_tdk = $arrRes[$num]['name'] . 'でおすすめのレンタルドレスショップ紹介';
                $this->tdk_name = $arrRes[$num]['name'];
                $pref_id = $arrRes[$num]['id'];
            }
        }

        //都道府県別ランキング 上位６位まで
        $todofuken_rank = "
        SELECT D.product_id,max(D.product_code) as product_code,
                max(P.photo_gallery_image1) as product_image, count(*) as order_cnt
            FROM dtb_order As O
            INNER JOIN dtb_order_detail  As D ON O.order_id = D.order_id
            INNER JOIN dtb_products As P ON D.product_id = P.product_id
            INNER JOIN mtb_pref As J ON CAST(o.order_pref as integer) = J.id
            WHERE O.del_flg<>1 and O.status not in (6,8)
            AND O.create_date >= ?
            AND O.create_date <= ?
            AND (P.status = 1)
            AND product_code like '11-%'
            AND O.order_pref = ? /*東京都*/
            Group By D.product_id,o.order_pref,J.name Order By order_cnt Desc limit 6";

        $today_is = date("Y-m-d H:i:s");
        $before_month = date("Y-m-d H:i:s",strtotime("-4 month"));
        //$today_is = '2021-3-1 00:00:00';
        //$three_month = '2021-7-29 23:59:59';
        $this->res_todofuken_rank = array();
        $this->tdk_pid = array();
        $this->res_todofuken_rank = $objQuery->getall($todofuken_rank, array($before_month, $today_is, $pref_id));
        foreach ($res_todofuken_rank as $key => $value) {
            array_push($this->tdk_pid, $res_todofuken_rank[$key]['product_id']);
        }

        //ワンピース,dressなどコーデのポイントをdescriptionに入れる
        if(strpos($_SERVER['REQUEST_URI'], 'detail.php?product_id') !== false){
            $product_num = $_GET['product_id'];
            $sql = "SELECT main_comment FROM dtb_products where product_id = ?";
            $arrRes = $objQuery->getall($sql, $product_num);
            if($arrRes[0]['main_comment']){
                $this->wanpi_comment = $arrRes[0]['main_comment'];    
            }else{
                $this->wanpi_comment = '';
            }
        }
    ...
    */
    ...
    function lfPreGetRecentProducts($product_id)
    {
        ...
        $arrRecent = array();
        $objQuery = new SC_Query();
        $sql = "SELECT T1.product_id , T1.name, T1.main_list_image, T2.price02, T2.product_code
        ...
        for ($i=0; $i < 6; $i++) { 
            $cate_id = $objQuery->select("category_id", "vw_products_allclass", "product_id = ? ORDER BY category_id ASC", array($arrRes[$i]['product_id']));
            array_push($arrRes[$i], $cate_id[0]);
        }
        ...
        foreach ($arrRecent as $key => $val)
        {
            $arrRecent[$key] = SC_Utils_Ex::productReplaceWebp($val);
        }

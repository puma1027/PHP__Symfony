...
//}}BHM_20140316
    ...
    public static function sfTax($price, $tax, $tax_rule)
    {
	   static $CONF;
        ...
        if (is_null($CONF) && (empty($tax) || is_null($tax_rule))) {
    ...
    //{{add BHM_20140313
        /* 規格の登録 */
    public function sfInsertProductClass($objQuery, $arrList, $product_id , $product_class_id = "")
    {
        ...
        $where = "product_id = ? ";
        $aryTemp = $objQuery->getRow( "classcategory_id1, classcategory_id2, wed_flag, shipping_date", "dtb_products_class", $where, array($product_id));
        $count = 0;
        if(count($aryTemp)>0 && (!empty($aryTemp['0']) || !empty($aryTemp['1']))){
            $count = 1;
           }
        // すでに規格登録がない場合
        if($count == 0) {
            ...
            $sqlval['wed_flag'] = $aryTemp['2'];
            $sqlval['shipping_date'] = $aryTemp['3'];
        ...
        // 特殊制御文字の手動エスケープ
   public function sfManualEscape($data) {
        // 配列でない場合
        if(!is_array($data)) {
            if (DB_TYPE == "pgsql") {
    ...
    function sfReload($get = "") {
        if ($_SERVER["SERVER_PORT"] == "443" ){
            $url = preg_replace('/' . URL_DIR . "$/", "", HTTPS_URL);
        } else {
            $url = preg_replace('/' . URL_DIR . "$/", "", SITE_URL);
        }
        if($get != "") {
        ...
        for ($i = 0; $i < count($array); $i++){
            ...
            if (is_array($arrayIndex) && 0 < count($arrayIndex)){
                for ($j = 0; $j < count($arrayIndex); $j++ ){
                    if ( $j > 0 ) $return .= ",";
                    $return .= "\"";
                    $return .= preg_replace("/</u", "＜", preg_replace( "/\\\"/u", "\\\"\\\"", $array[$i][$arrayIndex[$j]] ) ) ."\"";
                }
            } else {
                for ($j = 0; $j < count($array[$i]); $j++ ){
                    if ( $j > 0 ) $return .= ",";
                    $return .= "\"";
                    $return .= preg_replace("/</u", "＜", preg_replace( "/\\\"/u", "\\\"\\\"", $array[$i][$j] )) ."\"";
                }
            }
            $return .= "\n";
        }
...
// add ishibashi
     ...
     * 商品画像をwebpに変換
     */
    public function productReplaceWebp($product)
    {
        $browser = strtolower($_SERVER['HTTP_USER_AGENT']);

        if (self::checkSafari($browser) === false && self::checkVesion($browser) === false)
        {
            if (isset($product['main_image']))          $product['main_image']       = self::replaceWebp($product['main_image']);
            if (isset($product['main_large_image']))    $product['main_large_image'] = self::replaceWebp($product['main_large_image']);

            for ($i = 1; $i <= PRODUCTSUB_MAX; $i++)
            {
                if (isset($product['sub_image' . $i]))       $product['sub_image' . $i]        = self::replaceWebp($product['sub_image' . $i]);
                if (isset($product['sub_large_image' . $i])) $product['sub_large_image' . $i]  = self::replaceWebp($product['sub_large_image' . $i]);
            }

            $photoGalleryImageNum = PHOTO_GALLERY_IMAGE_NUM + 1;
            for ($n = 1; $n <= $photoGalleryImageNum; $n++)
            {
                if (isset($product['photo_gallery_image' . $n])) $product['photo_gallery_image' . $n] = self::replaceWebp($product['photo_gallery_image' . $n]);
            }

            if (isset($product['main_list_image'])) $product['main_list_image'] = self::replaceWebp($product['main_list_image']);
        }

        return $product;
    }

    public function replaceWebp($imageFileName)
    {
        $browser = strtolower($_SERVER['HTTP_USER_AGENT']);
        ...
        if (self::checkSafari($browser) === false && self::checkVesion($browser) === false)
        {
            if ($imageFileName !== null)
            {
                ...
                if(file_exists(IMAGE_SAVE_REALDIR . $webpFileName))
                {
    ...
    /*
     * 対応ブラウザ Safari on iOS 3.2~13.7 Safari 3.1~13.1
     */
    public function checkSafari($browser)
    {
        // ユーザーエージェントの情報を基に判定
        if (strstr($browser, 'edge')
            || strstr($browser, 'trident')
            || strstr($browser, 'msie')
            || strstr($browser, 'chrome')
            || strstr($browser, 'firefox')
            || strstr($browser, 'opera'))
        {
            return false;
        }
    ...
    public function checkVesion($browser)
    {
        $versionFlg = false;
        // iosはversionを変更する
        ...
        {
            if (strstr($val , 'version'))
            {
                $arrVersion = explode('/', $val);
                $version    = (float)$arrVersion[1];

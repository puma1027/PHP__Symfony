...
class SC_PageNavi_Border {
    ...
    // コンストラクタ
    function __construct($now_page, $all_row, $page_row, $func_name, $navi_max = NAVI_PMAX) {
        $this->arrPagenavi['mode'] = 'search';
        $ps = htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES);

        //現在ページ($now_page)が正しい数値でない場合
        if (!preg_match("/^[[:digit:]]+$/i", $now_page) || $now_page < 1 || $now_page == "") {
            $this->now_page = 1;
        } else {
        ...
        if($all_row > 1) {
            ...
            $next = "";
            if ($this->now_page > 1) {
                $before="<a href=\"". $ps . "\" onclick=\"$func_name('" . (($this->now_page) - 1) . "'); return false;\" style='text-decoration: none;'><span style='background-color:#FFFFFF;border: 1px solid #50382b;padding: 3px;'>&lt;&lt;前へ</span></a> ";
            ...
            if ($this->now_page < $this->max_page) {
                $next=" <a href=\"". $ps . "\" onclick=\"$func_name('" . (($this->now_page) + 1) ."'); return false;\" style='text-decoration: none;'><span style='background-color:#FFFFFF;border: 1px solid #50382b;padding: 3px;'>次へ&gt;&gt;</span></a>";
            ...
            for ($i=$disp_min; $i <= $disp_max; $i++) {
                ...
                if ($i == $this->now_page) {
                    $page_number .= "<strong style='background-color:#50382b;color:#ffffff;padding: 4px;'>$i</strong>";
                } else {
                    $page_number.="<a href=\"".  $ps . "\" onclick=\"$func_name('$i'); return false;\" style='text-decoration: none;'><span style='background-color:#FFFFFF;border: 1px solid #50382b;padding: 3px;'>$i</span></a>";
                }

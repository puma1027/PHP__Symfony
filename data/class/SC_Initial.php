...
//KHS END
    ...
    public function defineParameter()
    {
		...
		// ============== Add START===============
		// 独自の定数を設定
        if (is_file(CACHE_REALDIR . 'mtb_constants_custom_init.php')) {
            require_once CACHE_REALDIR . 'mtb_constants_custom_init.php';

            // キャッシュが無ければ, 初期データからコピー
        } elseif (is_file(CACHE_REALDIR . '../mtb_constants_custom_init.php')) {
            $mtb_constants = file_get_contents(CACHE_REALDIR . '../mtb_constants_custom_init.php');
            if (is_writable(CACHE_REALDIR)) {
                $handle = fopen(CACHE_REALDIR . 'mtb_constants_custom_init.php', 'w');
                if (!$handle) {
                    die($errorMessage);
                }
                if (fwrite($handle, $mtb_constants) === false) {
            ...
            die(CACHE_REALDIR . '../mtb_constants_custom_init.php が存在しません');
        }
    ...
    public function defineConstants()
    {
		// {{ Add MGN_20140310 
    	// 独自の定数を設定
        if (is_file(CACHE_PATH . "mtb_constants_custom.php")) {
            require_once(CACHE_PATH . "mtb_constants_custom.php");
            // キャッシュが無ければ, 初期データからコピー
        } elseif (is_file(CACHE_PATH . "../mtb_constants_custom_init.php")) {
            $mtb_constants = file_get_contents(CACHE_PATH . "../mtb_constants_custom_init.php");
            if (is_writable(CACHE_PATH)) {
                $handle = fopen(CACHE_PATH . "mtb_constants_custom.php", "w");
                if (!$handle) {
                    die($errorMessage);
                }
                if (fwrite($handle, $mtb_constants) === false) {
            ...
            die(CACHE_PATH . "../mtb_constants_custom_init.php が存在しません。");
        }
        // 配置ID
        /** 配置ID: 未使用 */

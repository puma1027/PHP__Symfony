...
class SC_SendMail
{
    ...
    //KHS Add 2014.3.17
    public $arrAttachFile;    // Attachement Files Info's array
    public $boundary;
    //KHS End
    /**
     * コンストラクタ
    ...
    public function __construct()
    {
        ...
        $this->port = SMTP_PORT;
        //KHS ADD 2014.3.17
        $this->arrAttachFile = null;
        $this->boundary = uniqid("next_part");
        //END
    ...
    public function setBody($body)
    {
        ...
        $this->body .= PHP_EOL;
         ...
         //KHS Add 2014.3.17
        if(!empty($this->arrAttachFile)){
            $this->body .= $this->_addAttachmentFile();
        }
        //END
    }
    //KHS Add 2014.3.17
    ...
    // 添付ファイルの設定
    function setAttachment($arrAttachFile) {
        $this->arrAttachFile = $arrAttachFile;
    }

    protected function _addAttachmentFile(){
        $body = "";
        $aryAttach = $this->arrAttachFile;
        $boundary = $this->boundary;
        for($i=0;$i<count($aryAttach);$i++) {
            if(!empty($aryAttach[$i])) {
                $filename = basename($aryAttach[$i]);  // filename
                $fp = fopen($aryAttach[$i], "r");     // file open
                $file = fread($fp, filesize($aryAttach[$i]));  // read file
...
/** 2014.6.26 RCHJ Add */
		if(!empty($this->arrAttachFile)){
        	$arrHeader['Content-Type-Multi'] = "multipart/mixed;";
        	$arrHeader['Boundary'] = "\"".$this->boundary."\"\r\n";
        	$arrHeader['--'] = $this->boundary;
        }
/** End */

...
__EOS__;
    ...
    // product_type毎の商品名一覧を取得する(select2用)
    public function getProductListsByProductType( &$objQuery, $product_type, $arrForm )
    {
        $res  = $objQuery->select('*', 'dtb_products', 'del_flg = 0 AND parent_flg = 1 AND product_type = ? ORDER BY product_id', [ $product_type ] );
        $tmp  = [];
        foreach( $res as $key => $val )
        {
            if ( isset( $arrForm['parent_product_id'] ) && (int)$arrForm['parent_product_id'] === (int)$val['product_id'] )
            {
                $tmp[] = [
                    'id' => $val['product_id']
                    ,'text' => $val['product_id'] . ': ' . $val['name']
                    ,'selected' => true
                ];
            }
            else
        ...
        return json_encode( $tmp, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT );
    }

    // 2020.10.14 SG.Yamauchi add
    // product_idから、商品情報、親商品として設定されている商品情報を取得する
    public function getGroupProductLists( &$objQuery, $parent_product_id, $own_product_id )
    {
        return $objQuery->select(
            '*, ( SELECT product_code FROM dtb_products_class WHERE dtb_products_class.product_id = dtb_products.product_id LIMIT 1 ) AS product_code'
            ,'dtb_products'
            ,'dtb_products.del_flg = 0 AND ( dtb_products.product_id = ? OR dtb_products.parent_product_id = ? ) AND dtb_products.product_id <> ?'
            ,[ $parent_product_id, $parent_product_id, $own_product_id ]
        );
    }
    // 2021 add ishibashi 商品ステータスの値を以前のステータスを揃える
    public function getProductFlag( $product_flag )
    {    
        $res = str_split($product_flag);
        
        foreach ( $res as $key => $val )
        {
            if ( $val === '1')
            {
                $productStatusFlag = $key+1;
            }
        }

        return $productStatusFlag;
    }
    // 20210201 add ishibashi 商品ステータスの値を以前のステータスを揃える
    public function convertProductFlag( $product_flag )
    {
        for ($i = 1; $i <= 5; $i++ )
        {
            if( (int)$product_flag === $i )
            {
                $res .= 1;
            }
            else

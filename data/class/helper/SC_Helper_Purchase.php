...
class SC_Helper_Purchase
{
    ...
    public function registerShipping($order_id, $arrParams, $convert_shipping_date = true)
    {
        ...
        foreach ($arrParams as $key => $arrShipping) {
                //KHS ADD 2014.4.4
                $order=array();
                      //$order['order_id']=$order_id;//$shipping['order_id'];
                      $order['deliv_name01']=$arrShipping['shipping_name01'];
                      $order['deliv_name02']=$arrShipping['shipping_name02'];
                      $order['deliv_kana01']=$arrShipping['shipping_kana01'];
                      $order['deliv_kana02']=$arrShipping['shipping_kana02'];
                      
                      $order['deliv_tel01']=$arrShipping['shipping_tel01'];
                      $order['deliv_tel02']=$arrShipping['shipping_tel02'];
                      $order['deliv_tel03']=$arrShipping['shipping_tel03'];
                      $order['deliv_fax01']=$arrShipping['shipping_fax01'];
                      $order['deliv_fax02']=$arrShipping['shipping_fax02'];
                      $order['deliv_fax03']=$arrShipping['shipping_fax03'];
                      
                      $order['deliv_pref']=$arrShipping['shipping_pref'];
                      $order['deliv_zip01']=$arrShipping['shipping_zip01'];
                      $order['deliv_zip02']=$arrShipping['shipping_zip02'];
                      
                      $order['deliv_addr01']=$arrShipping['shipping_addr01'];
                      $order['deliv_addr02']=$arrShipping['shipping_addr02'];
                      $order['deliv_time_id']=$arrShipping['time_id'];
                      $order['deliv_time']=$arrShipping['shipping_time'];
                      $order['deliv_no']=$arrShipping['shipping_num'];
                      $order['deliv_date']=$arrShipping['shipping_date'];
                      $order['commit_date']=$arrShipping['shipping_commit_date'];
                      
                      //$order['create_date']-$arrShipping['create_date'];
                      //$order['update_date']=$arrShipping['update_date'];
                      $order['sending_date']=$arrShipping['sending_date'];
                //KHS END
            //$arrValues = $objQuery->extractOnlyColsOf($table, $arrShipping);
            $arrValues = $objQuery->extractOnlyColsOf($table, $order);
            ...
            if (!SC_Utils_Ex::isBlank($arrValues['deliv_date'])
                && $convert_shipping_date) {
                $d = mb_strcut($arrValues['deliv_date'], 0, 10);
                $arrDate = explode('/', $d);
                $ts = mktime(0, 0, 0, $arrDate[1], $arrDate[2], $arrDate[0]);
                $arrValues['deliv_date'] = date('Y-m-d', $ts);
            }
    ...
    public function registerOrder($order_id, $arrParams)
    {
        ...
        if ($exists) {
			...
			if($arrValues['status'] == ORDER_STATUS_DELIV) {// correct=db:"発送済み" incorrect=constant:ORDER_DELIV(5)
				$arrValues['commit_date'] = 'CURRENT_TIMESTAMP';
			}
			...
			if ($arrValues['status'] == ORDER_STATUS_DELIV) {// correct=db:"発送済み"(3) incorrect=constant:ODERSTATUS_COMMIT(5)
				// 受注テーブルの発送済み日を更新する
				$arrValues['commit_date'] = 'CURRENT_TIMESTAMP';
			}
			unset($arrValues['total_point']);
			unset($arrValues['point']);
 ...
 //           $arrValues['order_detail_id'] = $objQuery->nextVal('dtb_order_detail_order_detail_id');
            $sql = 'SELECT MAX(order_detail_id) FROM dtb_order_detail ';
            $maxid=$objQuery->getOne($sql,'');
            if (is_null($maxid)) $maxid=0;
             $arrValues['order_detail_id']=$maxid+1;
            $arrValues['order_id'] = $order_id;
            $arrValues['classcategory_id1'] = 0;
            $arrValues['classcategory_id2'] = 0;
            $arrValues['change_flg'] = 0;
            $arrValues['change_date'] = null;
            //$arrValues['point_rate'] = 1;
            //KHS END
			// ============== 2014/0609 ADD   HWH ========================
			if(empty($arrValues['tax_rate'])){
				$arrValues['tax_rate'] = $CONF['tax'];
			}
        	if(empty($arrValues['tax_rule'])){
				$arrValues['tax_rule'] = $CONF['tax_rule'];
			}
			// ============== 2014/0609 END   HWH ========================
            $objQuery->insert($table, $arrValues);
        }
    ...
    public function getOrder($order_id, $customer_id = null)
    {
        ...
        //KHS Modify 2014.3.16
        //return $objQuery->getRow('*', 'dtb_order', $where, $arrValues);
        $ret=$objQuery->getRow('*', 'dtb_order', $where, $arrValues);
         //sending_date field remove
        $tempVal=array();
...
__EOS__;
    ...
    public function getShippings($order_id, $has_items = true)
    {
        ...
        $arrShippings = $objQuery->select('*', 'dtb_order', 'order_id = ?', array($order_id));
          // shipping_id ごとの配列を生成する
        foreach ($arrShippings as $index=>$order) {
         //KHS ADD 
            $shipping=array(); 
              $shipping['shipping_id']=$index;
              $shipping['order_id']=$order['order_id'];
              $shipping['shipping_name01']=$order['deliv_name01'];
              $shipping['shipping_name02']=$order['deliv_name02'];
              $shipping['shipping_kana01']=$order['deliv_kana01'];
              $shipping['shipping_kana02']=$order['deliv_kana02'];
              $shipping['shipping_company_name']=null;
              $shipping['shipping_tel01']=$order['deliv_tel01'];
              $shipping['shipping_tel02']=$order['deliv_tel02'];
              $shipping['shipping_tel03']=$order['deliv_tel03'];
              $shipping['shipping_fax01']=$order['deliv_fax01'];
              $shipping['shipping_fax02']=$order['deliv_fax02'];
              $shipping['shipping_fax03']=$order['deliv_fax03'];
              $shipping['shipping_country_id']=392;
              $shipping['shipping_pref']=$order['deliv_pref'];
              $shipping['shipping_zip01']=$order['deliv_zip01'];
              $shipping['shipping_zip02']=$order['deliv_zip02'];
              $shipping['shipping_zipcode']=null;
              $shipping['shipping_addr01']=$order['deliv_addr01'];
              $shipping['shipping_addr02']=$order['deliv_addr02'];
              $shipping['time_id']=$order['deliv_time_id'];
              $shipping['shipping_time']=$order['deliv_time'];
              $shipping['shipping_num']=$order['deliv_no'];
              $shipping['shipping_date']=$order['deliv_date'];
              $shipping['shipping_commit_date']=$order['commit_date'];
              $shipping['rank']=null;
              $shipping['create_date']=$order['create_date'];
              $shipping['update_date']=$order['update_date'];
              $shipping['del_flg']=$order['del_flg'];
              $shipping['sending_date']=$order['sending_date'];
        //KHS END
    ...
    public function getShipmentItems($order_id, $shipping_id, $has_detail = true)
    {
        ...
        $arrItems = $objQuery->select('*', 'dtb_order_detail ',
                                      'order_id = ?',
                                      array($order_id));                                      
        ...
        foreach ($arrItems as $key => $arrItem) {
            ...
            //KHS ADD 2014.4.4
            $shipment_item=array(); 
                $shipment_item['order_id']=$arrItem['order_id'];
                $shipment_item['product_name']=$arrItem['product_name'];
                $shipment_item['product_code']=$arrItem['product_code'];
                $shipment_item['classcategory_name1']=$arrItem['classcategory_name1'];
                $shipment_item['classcategory_name2']=$arrItem['classcategory_name2'];
                $shipment_item['price']=$arrItem['price'];
                $shipment_item['quantity']=$arrItem['quantity'];
                $shipment_item['shipping_id']=$shipping_id;
                $shipment_item['product_class_id']=$product_class_id;
             //KHS  END   
            foreach ($shipment_item as $detailKey => $detailVal) {
                $arrResults[$key][$detailKey] = $detailVal;
            }
            // 商品詳細を関連づける
            if ($has_detail && !empty($product_class_id)) {
        ...
        if (USE_POINT !== false) {
            ...
            if ($addCustomerPoint != 0) {
                ...
                if ($addCustomerPoint < 0) {
                    ...
                    var_dump('ポイントが'.$point);
                    var_dump($arrOrderOld['customer_id']);
                    // 変更後の会員.ポイントがマイナスの場合、
                    if ($point < 0) {
                        // ロールバック
                        $objQuery->rollback();
...
__EOS__;*/
        $sql_sub = <<< __EOS__
                SELECT deliv_time
                FROM dtb_delivtime
                WHERE time_id = dtb_order.deliv_time_id
                    AND deliv_id = dtb_order.deliv_id
__EOS__;
           
            $objQuery->update('dtb_order', array(), //update('dtb_shipping', array(),
                              $sql_where,
                              array($order_id),
                              //array('shipping_time' => "($sql_sub)"));  
                              array('deliv_time' => "($sql_sub)")); //KHS ADD 2014.4.4
        }

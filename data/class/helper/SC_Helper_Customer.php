...
/* -- 秘密の質問の更新がある場合は暗号化
        if ($arrData['reminder_answer'] == DEFAULT_PASSWORD or $arrData['reminder_answer'] == '') {
            ...
            if ($old_version_flag && $is_password_updated) {
                ...
                $arrData['reminder_answer'] = $reminder_answer;
            }
        } else {
            // 旧バージョン(2.11未満)からの移行を考慮
            if ($old_version_flag && !$is_password_updated) {
    ...
    public function sfCustomerEntryParam(&$objFormParam, $isAdmin = false)
    {
        ...
        if ($isAdmin) {
            ...
            $objFormParam->addParam('所持ポイント', 'point', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK'), 0);
             //{{KHS Add 20140311
             $objFormParam->addParam(  'きっかけ','route_name',INT_LEN,'n',array('NUM_CHECK') );
             $objFormParam->addParam(  'その他','other_route', LTEXT_LEN, 'KVa',array('SPTAB_CHECK' , 'MAX_LENGTH_CHECK'));
             //}}KHS end 20140311
        }

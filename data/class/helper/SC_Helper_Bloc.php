...
class SC_Helper_Bloc
{
    ...
    public function getBloc($bloc_id)
    {
        ...
        if (SC_Utils_Ex::isAbsoluteRealPath($arrRet['tpl_path'])) {
            ...
            // 202090611 sg nakagawa
			//if($this->device_type_id == DEVICE_TYPE_SMARTPHONE){
			//	$tpl_path = SC_Helper_PageLayout_Ex::getTemplatePath($this->device_type_id) .  $arrRet['tpl_path'];
			//}else{
    ...
    public function getList()
    {
        ...
        $table = 'dtb_bloc';
		$objQuery->setOrder('del_flg DESC');
    ...
    public function save($sqlval)
    {
        ...
        // 20200611 sg nakagawa 
		//if ($this->device_type_id == DEVICE_TYPE_SMARTPHONE){
		//	$arrValues['tpl_path'] = 'frontparts/bloc/' . $sqlval['filename'] . '.tpl';
		//} else {
		...
		if ($this->device_type_id == DEVICE_TYPE_SMARTPHONE){
        	$bloc_path = $bloc_dir . $sqlval['filename'] . '.tpl';
		} else {
    ...
    public function delete($bloc_id)
    {
        ...
        $arrExists = $this->getWhere('bloc_id = ? AND del_flg = 0', array($bloc_id));
//todo　確認 sg_nakagawa

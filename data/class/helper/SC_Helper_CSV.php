...
class SC_Helper_CSV
{
    ...
    public function sfGetCsvOutput($csv_id = '', $where = '', $arrVal = array(), $order = 'rank, no')
    {
        ...
        // 20160409 add ヤマトB2CSVの場合はIDを変更する
        if ($csv_id == 99) {
          $csv_id = 3;
        }
    ...
    public function cbOutputCSV($data)
    {
        ...
        if ($this->output_header) {
            ...
            mb_convert_variables('cp932//TRANSLIT', CHAR_CODE, $header); // 20200527 ishibashi
            fputcsv($this->fpOutput, $header);
            $this->output_header = false;
        }
        mb_convert_variables('cp932//TRANSLIT', CHAR_CODE, $data); // 20200527 ishibashi
    ...
    public function sfDownloadCsvFromSql($sql, $arrVal = array(), $file_head = 'csv', $arrHeader = null, $is_download = false)
    {
        ...
        if (is_array($arrHeader)) {
            mb_convert_variables('cp932//TRANSLIT', CHAR_CODE, $arrHeader); // 20200527 ishibashi
        ...
        $objQuery->doCallbackAllCsv(array(&$this, 'cbOutputCSV'), $sql, $arrVal, "", $file_head);
    ...
    public static function &fopen_for_output_csv($filename = 'php://output')
    {
        ...
        stream_filter_append($fp, 'convert.iconv.utf-8/cp932//TRANSLIT'); // 20200527 ishibashi

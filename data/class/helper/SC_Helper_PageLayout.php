...
class SC_Helper_PageLayout
{
    ...
    {
    	if ($device_type_id == DEVICE_TYPE_PC) {
        ...
        $objPage->tpl_mainpage = $this->getTemplatePath($device_type_id) . 'user_data/' . $arrPageData[0]['filename'] . '.tpl';
        ...
        if ( isset( $arrBlocs ) )
        {
            for ( $i = 0; $i < count($arrBlocs); $i++ )
            {
                if ( $arrBlocs[$i]['device_type_id'] === '2' )
                {
                    $arrBlocs[$i]['tpl_path'] = '/var/www/html/onepiece-rental.net/html/../data/Smarty/templates/sphone/frontparts/bloc/' . basename($arrBlocs[$i]['tpl_path']);
                }
            }
        }
        // 無効なプラグインのブロックを取り除く.
	...
	/*2014/03/12 pwb add*/
	public function sfGetGuidePageLayout(&$objPage, $preview = false, $url = '', $device_type_id = DEVICE_TYPE_PC)
    {
        // URLを元にページ情報を取得
        if ($preview === false) {
            // $url = str_replace(ROOT_URLPATH, '', $url);
        ...
        switch ($device_type_id) {
 		...
 		$objPage->tpl_mainpage =  HTML_REALDIR . USER_DIR . USER_PACKAGE_DIR . $templateName . "/" . $arrPageData[0]['filename'] . ".tpl";
		// $objPage->tpl_mainpage =  "/virtual/www/eccube/html/user_data/packages/wanpi/maternity.tpl";
        if (!file_exists($objPage->tpl_mainpage)) {
            $msg = 'メイン部のテンプレートが存在しません。[' . $objPage->tpl_mainpage . ']';
            trigger_error($msg, E_USER_WARNING);
        }

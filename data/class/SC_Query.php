...
class SC_Query
{
	...
	public function doCallbackAllCsv($cbFunc, $sql, $arrVal = array(), $fetchmode = MDB2_FETCHMODE_ASSOC, $fg)
    {
        ...
        while ($data = $affected->fetchRow($fetchmode)) {
            if ($fg == "order"){
                //ゆうパックプリントのバージョンがあがった影響で数字が消えてしまわないよう、郵便番号には「'」をつける。
                $data['deliv_zip01'] = "'".$data['deliv_zip01'];
                $data['deliv_zip02'] = "'".$data['deliv_zip02'];

                //エクセル開いた後、日付に変換されちゃうことがあるので「'」をつける。
                $data['deliv_addr02'] = "'".$data['deliv_addr02'];

                $tmp = $data['sending_date'];

                //お客様への配達日と返却日を作成する
                $data['sending_date'] = date('Y', strtotime($tmp."+1 days"));
                $data['sending_date1'] = date('m', strtotime($tmp."+1 days"));
                $data['sending_date2'] = date('d', strtotime($tmp."+1 days"));
                $data['sending_date3'] = date('Y', strtotime($tmp."+5 days"));
                $data['sending_date4'] = date('m', strtotime($tmp."+5 days"));
                $data['sending_date5'] = date('d', strtotime($tmp."+5 days"));
            }
            ...
            if ($fg == "order_new"){
                ...
                // sending_date2にお届け日
                $data['sending_date2'] = $data['sending_date']."/".$data['sending_date1']."/".$data['sending_date2'];
                $sending_date1 = strtotime($data['sending_date2']);
                $sending_date1= strtotime( "-1 day", $sending_date1);
                // sending_date1に発送日
                $data['sending_date1'] = date("Y/m/d", $sending_date1);

                // 電話番号はハイフンで連結
                $data['order_tel01'] = $data['order_tel01']."-".$data['order_tel02']."-".$data['order_tel03'];

                //お届け時間
                if ($data['deliv_time'] == '午前中') {
                    $data['deliv_time'] = '0812';
                } else if ($data['deliv_time'] == '17時まで') {
                    $data['deliv_time'] = '0017';
                } else if ($data['deliv_time'] == '14時～16時') {
                    $data['deliv_time'] = '1416';
                } else if ($data['deliv_time'] == '16時～18時') {
                    $data['deliv_time'] = '1618';
                } else if ($data['deliv_time'] == '18時～20時') {
                    $data['deliv_time'] = '1820';
                } else if ($data['deliv_time'] == '19時～21時') {
                    $data['deliv_time'] = '1921';
                }

                $addr_split1 = mb_substr($data['deliv_addr02'], 0, 16, "utf-8");
                $addr_split2 = mb_substr($data['deliv_addr02'], 16, 41, "utf-8");
                $addr_split3 = mb_substr($data['deliv_addr02'], 43, 66, "utf-8");

                // 20160410 add ヤマトB2 住所を分割して格納
                $data["sending_date3"] = "";
                // 20160410 add ヤマトB2 住所を分割して格納
                $data["sending_date4"] = "";
                // 20160410 add ヤマトB2 住所を分割して格納
                $data["sending_date5"] = "";
                $arrDelivAddr = $this->mb_str_split($data['deliv_addr02']);
                for($i = 0; $i < count($arrDelivAddr); $i++) {

                    if ($i <= 15) {
                        $data["sending_date3"] = $data["sending_date3"]. $arrDelivAddr[$i];
                    }

                    if ($i >  15 && $i <= 40) {
                        $data["sending_date4"] = $data["sending_date4"]. $arrDelivAddr[$i];
                    }

                    if ($i >  40) {
                        $data["sending_date5"] = $data["sending_date5"]. $arrDelivAddr[$i];
                    }
                }
            }
     ...
     * @return SC_Query 自分自身のインスタンス
     */
	 public function setlimitoffset_old($limit, $offset = 0, $return = false) {
	...
	public function insert1($table, $sqlval) {
        $strcol = '';
        $strval = '';
        $find = false;
    ...
    public function update($table, $arrVal, $where = '', $arrWhereVal = array(), $arrRawSql = array(), $arrRawSqlVal = array())
    {
	 ...
	 public function update1($table, $sqlval, $where = "", $arradd = "", $addcol = "") {
        $strcol = '';
        $strval = '';
        $find = false;
        foreach ($sqlval as $key => $val) {
            if(preg_replace("/^Now\(\)$/i", $val))
            {
                $strcol .= $key . '= Now(),';
            // 先頭に~があるとプレースホルダーしない。
            }

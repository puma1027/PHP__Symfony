...
class SC_Display
{
	...
	public function prepareProcess($is_admin = false) {
		$force_result = true;
		if (!$this->deviceSeted || !is_null($this->view)) {
			$device = ($is_admin) ? DEVICE_TYPE_ADMIN : $this->detectDevice();
			...
			if (isset($_SESSION['force_device'])){
				$force_device = trim(strtolower($_SESSION['force_device']));
			}
			if (isset($_REQUEST["show_view"])){
				$force_device = trim(strtolower($_REQUEST['show_view']));
				$_SESSION['force_device'] = $force_device;
			}
			switch ($force_device){
				case "pc":
    ...
    public static function detectDevice($reset = FALSE)
    {
		...
		if($_SESSION['force_device']!='')
        {
			switch ($_SESSION['force_device'])
            {
            case "pc":
                $device = DEVICE_TYPE_PC;
                break;

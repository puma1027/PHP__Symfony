...
__EOS__;
	...
	/*	$sql.= "to_char(sending_date, '".$format."') AS str_date,";
        $sql.= "COUNT(order_id) AS total_order, ";
        $sql.= "(SELECT COUNT(*) AS cnt $from $where AND T1 .customer_id = 0) AS nonmember, ";
        ...
        //::$sql.= "SUM(total) AS total, ";//::N00055 Del 20130523
        $sql.= "SUM(payment_total) AS total, ";//::N00055 Add 20130523
        $sql.= "(AVG(total)) AS total_average ";*/

		$sql = "to_char(sending_date, '".$format."') AS str_date, sum(total_order) as total_order, sum(men) as men, sum(women) as women, sum(men_member) as men_member, sum(men_nonmember) as men_nonmember, sum(women_member) as women_member, sum(women_nonmember) as women_nonmember, sum(total) as total, sum(total_average) as total_average ";

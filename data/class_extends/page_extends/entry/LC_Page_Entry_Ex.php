...
class LC_Page_Entry_Ex extends LC_Page_Entry
{
    ...
    function init()
    {
        ...
        $this->arrVisitRouteName = array("日テレ1分間の深イイ話から","NHK東京カワイイTVから","「結婚式の服装マナー」のページから","「結婚式二次会のマナー」のページから","「マタニティドレス」のページから","「レンタルドレス」で検索したら出てきた","知人・友人の紹介");//::N00044 Add 20130501
        //::$this->arrVisitRouteName = $masterData->getMasterData("mtb_visit_route");//::N00044 Add 20130506
    }
...
//kiyaku
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $_POST["other_route"] = preg_replace('/^[ 　\r\n]*(.*?)[ 　\r\n]*$/u', '$1', $_POST["other_route"]);//::N00044 Add 20130501
            ...
            if ($this->arrErr || $_POST["mode"] == "return") {		// 入力エラーのチェック
                ...
                if ($this->getMode() === 'complete' )
                {
                    ...
                    //　完了メール送信
                    $this->name01 = $this->arrForm['name01'];
                    $this->name02 = $this->arrForm['name02'];
                    ...
                    // 宛先の設定
                    $name = $this->arrForm["name01"] . $this->arrForm["name02"] ." 様";
                    $objMail->setTo($_POST["email"], $name);
                    $objMail->sendMail();

                    // 完了ページに移動させる。
                    $customer_id = $objQuery->get("customer_id", "dtb_customer", "secret_key = ?", array($this->uniqid));
                    SC_Response_Ex::sendRedirect($this->getLocation("./complete.php", array("ci" => $customer_id)));
    ...
    function mobileProcess() {
        ...
        $arrRegistColumn = array(
                                 ...
                                 array(  "column" => "password02", "convert" => "a" ),
                                 array(  "column" => "mailmaga_flg", "convert" => "n" ),
                                 array(  "column" => "route_name", "convert" => "n" ),//::N00044 Add 20130501
                                 array(  "column" => "other_route", "convert" => "aKV"),//::N00044 Add 20130501
                                 );
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if ($this->arrErr || !empty($_POST["return"])) {		// 入力エラーのチェック
                ...
                //-- データの設定
                if ($_POST["mode"] == "set1") {
                    $checkVal = array("email", "password", "name01", "name02", "kana01", "kana02");
                } elseif ($_POST["mode"] == "set2") {
                    $checkVal = array("kiyaku", "year", "month", "day", "zip01", "zip02");
                } else {
                ...
                if ($_POST["mode"] == "set1") {
                    ...
                    //パスワード表示
                    $passlen = strlen($this->arrForm['password']);
                    $this->passlen = $this->sfPassLen($passlen);
                ...
                if ($_POST["mode"] == "complete") {
                    ...
                    if (isset($_SESSION['mobile']['kara_mail_from'])) {
                        ...
                        SC_Response_Ex::sendRedirect($this->getLocation(MOBILE_URL_DIR . "regist/index.php", $param));
                        SC_Response_Ex::actionExit();
                    }
                    ...
                    // 完了ページに移動させる。
                    SC_Response_Ex::sendRedirect($this->getLocation("./complete.php"), true);
                    SC_Response_Ex::actionExit();
                }
            }
        }
    ...
    function lfErrorCheck($array) {
        ...
        $objErr->doFunc(array("ご利用規約", "kiyaku") ,array("SELECT_CHECK", "NUM_CHECK"));
        ...
        $objErr->doFunc(array("パスワード(確認)", 'password02', PASSWORD_MIN_LEN, PASSWORD_MAX_LEN), array("EXIST_CHECK", "SPTAB_CHECK" ,"ALNUM_CHECK", "NUM_RANGE_CHECK"));

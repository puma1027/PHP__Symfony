...
class LC_Page_Shopping_Payment_Ex extends LC_Page_Shopping_Payment
{
    ...
    {
        parent::init();
        $this->tpl_mainpage = 'shopping/payment_wide.tpl';//::N00039 Add 20130430
...
//        $this->allowClientCache();
    ...
    function action()
    {
       ...
       // ユーザユニークIDの取得と購入状態の正当性をチェック
        ...
        $this->tpl_mainpage = 'shopping/payment_wide.tpl';
        switch($this->getMode()) {
        case 'confirm':
            // 入力値の変換
        ...
        $arrOrder = $objDb->sfGetOrderTemp($uniqid);

        // 支払方法を決定する(地域と商品数により変わる)
        $confirmPayment = SC_Helper_Payment_Ex::sfConfirmPayment($this->arrPayment, $this->arrProductsClass, $arrOrder);
        // 一旦消去
        unset($this->arrPayment);
        // 補完
        $this->arrPayment = $confirmPayment;

        // 配送時間の取得
        // TODO お支払方法は一つしか存在しない前提
        $arrRet = $objDb->sfGetDelivTime($this->arrPayment[0]['payment_id']);
        $this->arrDelivTime = SC_Helper_Delivery_Ex::sfGetSummaryDelivTime($arrOrder['deliv_pref'], $arrRet);
        $this->arrDelivTime = SC_Utils_Ex::sfArrKeyValue($this->arrDelivTime, 'time_id', 'deliv_time');

// ============== 2012.05.28 RCHJ Remark & Add =============
        /*// 配送日一覧の取得
        $objDate = new SC_Helper_Date_Ex();
        $this->arrDelivDate = $objDate->lfGetDelivDateAutoSelect();

        // 木曜お届けOKの商品判定
       	$deliv_day = $objDate->getDelivDay($this->arrDelivDate);
        $this->tpl_delive_wed = true;
        foreach ($this->arrProductsClass as $aryProduct){
        	//$this->tpl_delive_wed = $this->isValidProduct($aryProduct["product_class_id"], $deliv_day);
        	$this->tpl_delive_wed = $objDate->isValidProduct($aryProduct["product_id"], $deliv_day);
        	if($this->tpl_delive_wed == false){
        		break;
        	}
        }
        */
        ...
        /*// 木曜お届け不可(火曜の15時以降)に木曜お届けで注文した場合
        if ($this->rejectDelivDate == true) {
            unset($this->arrForm['deliv_date']);
        }

        // 同梱処理
        $this->tpl_all_deliv_disiable = false;
        if(isset($_SESSION["cart"]["dongbong_info"]) && $_SESSION["cart"]["dongbong_info"]["flag"]){
			$this->arrForm['deliv_date']['value'] = $_SESSION["cart"]["dongbong_info"]["deliv_date"];
			$this->tpl_all_deliv_disiable = true;
        }*/

        $objReserveUtil = new SC_Reserve_Utils();
        $ary_rental_day = $objReserveUtil->getRentalDay($_SESSION["cart"]["send_date"]);
        $this->tpl_deliv_date = $ary_rental_day["arrival_day"];
        // ============== End =============
    ...
    function lfInitParam() {
        ...
        $this->objFormParam->addParam("ポイント", "use_point", INT_LEN, "n", array("MAX_LENGTH_CHECK"));// , "NUM_CHECK", "ZERO_START" 2012.07.25 RCHJ Change
        ...
        $this->objFormParam->addParam("配達日", "deliv_date", STEXT_LEN, "KVa", array("EXIST_CHECK", "MAX_LENGTH_CHECK"));

		// KMS2014/01/21
		$this->objFormParam->addParam("メッセージカードの指名", "writer_select", INT_LEN, "n", array("MAX_LENGTH_CHECK"));
    }
    ...
    function lfCheckError($arrData) {
        ...
        /* 2012.05.28 RCHJ Remark
        // 木曜お届け不可(火曜の15時以降)チェック
        // 画面に入ったときは火曜の15時以前で出るときに15時を過ぎていないか確認
        $_result =  strstr ($_POST['deliv_date'],"木");
        $this->rejectDelivDate = false;
        if($_result){
        	$objDate = new SC_Helper_Date_Ex();
            if ($objDate->lfDelivPossibleDefineDay()==false){
                $objErr->arrErr['deliv_date']  = "現在時刻は木曜日のお届けはできません。";
                $this->rejectDelivDate = true;
            }
        }*/
        $objView = new SC_MobileView();
        $objSiteInfo = $objView->objSiteInfo;
    ...
    function lfRegistData($uniqid) {
    	...
    	/* 2012.05.28 RCHJ Remark
    	if(isset($_SESSION["cart"]["dongbong_info"]) && $_SESSION["cart"]["dongbong_info"]["flag"]){
			$sqlval['deliv_date'] = $_SESSION["cart"]["dongbong_info"]["deliv_date"];
        }
        */
        ...
        //::N00045 Add 20130430
        if ($_POST['writer_select'] == '1') {
            $sqlval['writer_select'] = 1;
        } elseif($_POST['writer_select'] == '2') {
            $sqlval['writer_select'] = 2;
        } elseif($_POST['writer_select'] == '3') {
            $sqlval['writer_select'] = 3;
        } elseif($_POST['writer_select'] == '4') {
            $sqlval['writer_select'] = 4;
        } elseif($_POST['writer_select'] == '5') {
            $sqlval['writer_select'] = 5;
        } else {
            $sqlval['writer_select'] = 10;
        }
        //::N00045 end 20130430
    ...
    function lfSetOrderTempData($uniqid) {
        ...
        $objQuery = new SC_Query();
        $col = "payment_id, use_point, deliv_time_id, message, point_check, deliv_date, writer_select";// KMS2014/01/21
        $from = "dtb_order_temp";
        $where = "order_temp_id = ?";
     ...
     */
    function lfCheckDelivArea($objCartSess){
        $objDb = new SC_Helper_DB_Ex();
        //::N00083 Change 20131201
        // カートの内の商品IDを取得
        //::$arrProductsId = $objCartSess->getAllProductID($objCartSess);
        //カート内の全商品情報を取得
        $arrSetPtype = array();
        $arrCartList = $objCartSess->getCartList();
        foreach ($arrCartList as $key=>$val) {
            // カートの内の商品IDを取得
            $arrProductsId[$key] = $val['id'][0];
            //セット商品のドレスのproduct_typeを取得する
            if ($val['id'][0] == $val['set_pid']) {
                $arrSetPtype[$val['id'][0]] = $val['set_ptype'];
            }
        }
        //::N00083 end 20131201

        // 配列であることと数値の確認
        if (is_array($arrProductsId) && count($arrProductsId) != 0) {
            foreach ($arrProductsId as $productId) {
                // カテゴリIDを取得
                $categoryId = $this->lfGetRootCtegory($productId);
                // 4点セットあり
                //::if ($categoryId == CATEGORY_DRESS4) {
                if (($categoryId == CATEGORY_SET_DRESS) && ($arrSetPtype[$productId] == 4)) {//::N00083 Change 20131201
                    return true;
                }
            }
        }

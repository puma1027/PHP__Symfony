...
class LC_Page_Admin_Products_Product_Ex extends LC_Page_Admin_Products_Product
{
    ...
    public function lfRegistProduct(&$objUpFile, &$objDownFile, $arrList)
    {
        ...
        */                            
        ...
        $checkArray = array("name", "status", "icon_flag","product_flag","map_url",
...
//        $objUpFile->addFile("商品詳細ファイル", 'file2', array('pdf'), PDF_SIZE, false, 0, 0, false);
        ...
        for ($cnt = 1; $cnt <= PHOTO_GALLERY_IMAGE_NUM-3; $cnt++) {
            $objUpFile->addFile("フォトギャラリー画像$cnt", "photo_gallery_image$cnt", array('jpg', 'gif', 'png'), IMAGE_SIZE, false, LARGE_IMAGE_WIDTH, LARGE_IMAGE_HEIGHT);
        }
        for ($cnt = PHOTO_GALLERY_IMAGE_NUM-2; $cnt <= PHOTO_GALLERY_IMAGE_NUM; $cnt++) {
            ...
            $objUpFile->addFile("詳細-サブ画像".($cnt - 11), "photo_gallery_image$cnt", array('jpg', 'gif', 'png'), IMAGE_SIZE, false, LARGE_IMAGE_WIDTH, LARGE_IMAGE_HEIGHT);
        }
    }
...
//        $objErr->doFunc(array("セット内容", "set_content", MTEXT_LEN), array("MAX_LENGTH_CHECK"));

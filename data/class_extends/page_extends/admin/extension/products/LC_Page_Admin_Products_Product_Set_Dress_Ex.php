...
// {{{ requires
require_once(CLASS_REALDIR . "pages/admin/extension/products/LC_Page_Admin_Products_Product_Set_Dress.php");

/**
 * 商品登録 のページクラス(拡張).
 *
 * LC_Page_Admin_Products_Product_Set_Dress をカスタマイズする場合はこのクラスを編集する.
 *
 * @package Page
 * @author EC-CUBE CO.,LTD.
 * @version $Id: 
 */
class LC_Page_Admin_Products_Product_Set_Dress_Ex extends LC_Page_Admin_Products_Product_Set_Dress {
...
$objErr->doFunc(array("モデル身長", "model_body_length", INT_LEN), array("NUM_CHECK")); // 2013.01.21 RCHJ Add（モデルの身長）
        //::N00072 Add 20130902
        $objErr->doFunc(array("こんな方におすすめ", "recommended_staff_comment", LLTEXT_LEN), array("SPTAB_CHECK", "MAX_LENGTH_CHECK"));
        $objErr->doFunc(array("こんな方におすすめ", "recommended_staff_comment", $this->arrAllowedTag), array("HTML_TAG_CHECK"));
        $objErr->doFunc(array("コーデのポイント", "coord_point_staff_comment", LLTEXT_LEN), array("SPTAB_CHECK", "MAX_LENGTH_CHECK"));
        $objErr->doFunc(array("コーデのポイント", "coord_point_staff_comment", $this->arrAllowedTag), array("HTML_TAG_CHECK"));
        $objErr->doFunc(array("オトコ目線レビュー", "mens_review_staff_comment", LLTEXT_LEN), array("SPTAB_CHECK", "MAX_LENGTH_CHECK"));
        $objErr->doFunc(array("オトコ目線レビュー", "mens_review_staff_comment", $this->arrAllowedTag), array("HTML_TAG_CHECK"));
        //::N00072 end 20130902
...
//       //{{add bhm_20140318   在庫0個で登録できないように
//       elseif ($array['stock'] == 0) {
//    $objErr->arrErr['stock'] = "在庫0個では登録できません。<br />";
       //}}add bhm_20140318
//    }
         else {
        ...
        $objErr->doFunc(array("付属品(ドレス)", "set_content", MTEXT_LEN), array("MAX_LENGTH_CHECK"));
        //Y.C add 2012/04/28
        $objErr->doFunc(array("付属品(羽織物)", "set_content1", MTEXT_LEN), array("MAX_LENGTH_CHECK"));
        $objErr->doFunc(array("付属品(ネックレス)", "set_content2", MTEXT_LEN), array("MAX_LENGTH_CHECK"));
        $objErr->doFunc(array("付属品(バッグ)", "set_content3", MTEXT_LEN), array("MAX_LENGTH_CHECK"));
    ...
    function lfRegistProduct($arrList) {
        ...
        $checkArray = array("name", "status", "map_url", /*R.K 2012/02/14*/"icon_flag","funct_flag","silhouette_flag", "product_flag",// *UAssist
							...
							, "image_dress", "image_stole", "image_necklace", "image_bag"
                            ,"bag_temp_id", "model_body_length"
                            ,"size_supplement1","size_supplement2","size_supplement3","size_supplement4","size_supplement5"
                            ,"recommended_staff_id","coord_point_staff_id","mens_review_staff_id"
                            ,"recommended_staff_comment","coord_point_staff_comment","mens_review_staff_comment"
                            ,"mpsc_age","mpsc_event","mpsc_size","mpsc_complex","mpsc_worry"
...
$sqlval['model_body_length'] = $arrList['model_body_length']; // 2013.01.21 RCHJ Add （モデル身長登録）
        //Y.C add 2012/04/28
        $sqlval['bag_temp_id'] = $arrList['bag_temp_id'];
        //::N00072 Add 20130807
		$sqlval['recommended_staff_id'] = $arrList['recommended_staff_id'];
		$sqlval['coord_point_staff_id'] = $arrList['coord_point_staff_id'];
		$sqlval['mens_review_staff_id'] = $arrList['mens_review_staff_id'];
        $sqlval['recommended_staff_comment'] = $arrList['recommended_staff_comment'];
        $sqlval['coord_point_staff_comment'] = $arrList['coord_point_staff_comment'];
        $sqlval['mens_review_staff_comment'] = $arrList['mens_review_staff_comment'];
        $sqlval['mpsc_age'] = $arrList['mpsc_age'];
        $sqlval['mpsc_event'] = $arrList['mpsc_event'];
        $sqlval['mpsc_size'] = $arrList['mpsc_size'];
        $sqlval['mpsc_complex'] = $arrList['mpsc_complex'];
        $sqlval['mpsc_worry'] = $arrList['mpsc_worry'];
		$sqlval['size_supplement1'] = $arrList['size_supplement1'];
		$sqlval['size_supplement2'] = $arrList['size_supplement2'];
		$sqlval['size_supplement3'] = $arrList['size_supplement3'];
		$sqlval['size_supplement4'] = $arrList['size_supplement4'];
		$sqlval['size_supplement5'] = $arrList['size_supplement5'];
        //::N00072 end 20130807
...
//====== RCHJ Add ========
        ...
        $product_type = SET_DRESS_PRODUCT_TYPE; // レンタルドレス4点セット(LC_Page_Admin_Products.php: Line 369)
        ...
        $sqlval["product_id"] = $product_id;
        $sqlval["product_type"] = $product_type;
        $sqlval["image_id1"] = $arrList["image_dress"];
        $sqlval["image_id2"] = $arrList["image_stole"];
        $sqlval["image_id3"] = $arrList["image_necklace"];
        $sqlval["image_id4"] = $arrList["image_bag"];
        $sqlval["creator_id"] = $_SESSION["member_id"];
        $sqlval["create_date"] = "Now()";
        $sqlval["update_date"] = "Now()";

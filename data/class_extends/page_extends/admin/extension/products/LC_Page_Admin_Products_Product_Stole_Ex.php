 ...
 * ####################################################
 * バージョン　変更日　		変更者　変更内容
 * 1.0.1	  2012/02/14	R.K		ストール・ボレロ登録用追加
 * ####################################################
 */

// {{{ requires

require_once(CLASS_REALDIR . "pages/admin/extension/products/LC_Page_Admin_Products_Product_Stole.php"); 
 ...
 * @version $Id$
 */
class LC_Page_Admin_Products_Product_Stole_Ex extends LC_Page_Admin_Products_Product_Stole {
...
// ======================= end =======================
    ...
    function lfRegistProduct($arrList) {
        ...
        // 配列の添字を定義
        $checkArray = array("name", "status", "map_url", /*R.K 2012/02/14*/"icon_flag", "product_flag",// *UAssist
							...
							,"releaseday_id","brand_id","main_comment_point", "image_stole"
							,"tag","wear_comment_model1","wear_comment_wearrank1","wear_comment1"
							...
							,"wear_comment_under2","main_comment_point","silhouette","funct", "image_dress", "model_body_length"
...
//R.K　2012/02/14　追加終了
        ...
        //::N00083 Add 20131202
        //変更前の商品コードを取得しておく
        $arrRet_product_code = $objQuery->select("product_code", "dtb_products_class", "product_id = ?", array($_POST['product_id']));
        //::N00083 end 20131202
        ...
        $sqlval2 = array();
        // UPDATEの実行
        $sqlval2['set_pcode_stole'] = $arrList["product_code"];
        $where = "set_pcode_stole = ?";
        $objQuery->update("dtb_products_class", $sqlval2, $where, array($arrRet_product_code[0]['product_code']));
        //::N00083 end 20131202
...
//====== RCHJ Add ========
        ...
        $product_type = STOLE_PRODUCT_TYPE; // ストール・ボレロを選ぶ(LC_Page_Admin_Products.php: Line 369)

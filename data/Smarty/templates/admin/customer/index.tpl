...
<!--
    ...
    function fnRentalTicketDownload(customer_id) {
        document.form1.mode.value = "rental_ticket_download"
        document.form1['download_customer_id'].value = customer_id;
        document.form1.submit();
        return false;
    }
//-->
...
<div id="customer" class="contents-main">
    ...
    <!--{if count($arrErr) == 0 and ($smarty.post.mode == 'search' or $smarty.post.mode == 'delete' or $smarty.post.mode == 'resend_mail' or $smarty.post.mode == 'rental_ticket_download') }--><!--{*//::KHS Add 2014.3.12 rental_ticket_download*}-->
        ...
        <form name="form1" id="form1" method="post" action="?">
            ...
            <input type="hidden" name="download_customer_id" value=""><!--{*//::N00068 Add 20140114*}-->
            <!--{foreach key=key item=item from=$arrHidden}-->
                <!--{if $key ne "mode" && $key ne "del_mode" && $key ne "edit_customer_id" && $key ne "del_customer_id" && $key ne "search_pageno" && $key ne "csv_mode" && $key ne "job" && $key ne "sex" && $key ne "status" && $key ne "download_customer_id"}-->
                    <input type="hidden" name="<!--{$key|escape}-->" value="<!--{$item|escape}-->">
                <!--{/if}--><!--{*//::N00068 Add 20140114 download_customer_id*}-->
                <!--{if is_array($item)}-->
            ...
            <table class="list" id="customer-search-result">
                ...
                <tr>
                    ...
                    <th rowspan="2">編集</th>
                    <th rowspan="2">削除</th>
                    <th rowspan="2">ﾚﾝﾀﾙ票</th><!--{*//::N00068 Add 20140114*}-->
                </tr>
                <tr>
                    <th>都道府県</th>
                    <th>メールアドレス</th>
                ...
                <!--{foreach from=$arrData item=row}-->
                    <tr>
                        ...
                        <td class="center" rowspan="2"><span class="icon_copy"><a href="#" onclick="return fnRentalTicketDownload('<!--{$row.customer_id|h}-->');">表示</a></span></td><!--{*//::KHS Add 2014.3.12*}-->
                    </tr>
                    <tr>

...
<form name="form1" id="form1" method="post" action="?" enctype="multipart/form-data">
    ...
    <div id="products" class="contents-main">
		...
		<!-- {{ADD KGS_20140307 -->
		    <select name="group_id" style="<!--{$arrErr.group_id|sfGetErrorColor}-->">
		    <option value="" selected="">指定なし</option>
		    <!--{html_options options=$arrGroup selected=$arrForm.group_id}-->
		    </select>
        <!-- ADD KGS_20140307}} -->
            ...
            <table class="list" id="categoryTable">
                ...
                <tr class="nodrop nodrag">
                    <th>ID</th>
                    <th>カテゴリ名(グループ)</th> <!-- ADD KGS_20140307 -->
                    <th class="edit">編集</th>
                ...
                <tr id="<!--{$arrList[cnt].category_id}-->" style="background:<!--{if $arrForm.category_id != $arrList[cnt].category_id}-->#ffffff<!--{else}--><!--{$smarty.const.SELECT_RGB}--><!--{/if}-->;" align="left">
                    ...
                    <!--{if $arrList[cnt].level != $smarty.const.LEVEL_MAX}-->
                    	...
                    	<!-- {{ADD KGS_20140307 -->
                        <!--{if $arrList[cnt].group_id ne ""}-->
                            (<!--{$arrList[cnt].group_id|escape}-->)
                        <!--{/if}-->
                        <!-- ADD KGS_20140307}} -->
                    <!--{else}-->

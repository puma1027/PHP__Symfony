...
function fnCustomModeSubmit(mode, keyname, keyid) {
	switch(mode) {
	case 'totalization':
		if(!window.confirm('集計開始します。宜しいですか。')){
			return;
		}
		break;
	default:
...
<!-- {{ADD KGS_20140307-->
	        <tr>
				<th>集計開始</th>
				<td colspan="3" >
					<input type="button" name="subm" value="集計開始" onclick="fnCustomModeSubmit('totalization','','');"  />
				</td>
			</tr>
            <tr>
				<th>★5つ評価</td>
    ...
    <!--{if count($arrErr) == 0 and ($smarty.post.mode == 'search' or $smarty.post.mode == 'delete')}-->
        ...
        <form name="form1" id="form1" method="post" action="?">
            ...
            <div class="btn">
                ...
                <!--{/if}-->
                <a class="btn-normal" href="javascript:;" onclick="eccube.fnFormModeSubmit('form1','csv','',''); return false;"><span>CSV ダウンロード</span></a>
<!--{*
todo 確認nakagawa
...
*}-->
            ...
            <!--{if $arrReview > 0 & $tpl_linemax > 0}-->
                ...
                <table id="products-review-result" class="list">
                    <tr>
                        <th>発送日</th> <!-- ADD KGS_20140307 -->
                        <th>投稿者名</th>
                        <th>商品名</th>
                        <th>おすすめレベル</th>
                        <th>表示・非表示</th>
                        <th>編集</th>
			            <th>コメント</th> <!-- ADD KGS_20140307 -->
                        <th>削除</th>
                    </tr>
                    ...
                    <!--{section name=cnt loop=$arrReview}-->
                        <tr>
			                ...
			                <td class="menu"><a href="javascript:;" onclick="eccube.changeAction('./review_recomment_edit.php'); eccube.setModeAndSubmit('','review_id','<!--{$arrReview[cnt].review_id}-->'); return false;">編集</a></td>	<!-- ADD KGS_20140307 -->

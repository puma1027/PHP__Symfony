...
<form name="form1" id="form1" method="post" action="?">
    ...
    <div id="basis" class="contents-main">
        ...
        <table class="list">
            ...
            <!--{section name=cnt loop=$arrNormalday}-->
            <tr style="background:<!--{if $tpl_day_id != $arrNormalday[cnt].day_id}-->#ffffff<!--{else}--><!--{$smarty.const.SELECT_RGB}--><!--{/if}-->;">
                <!--{assign var=day_id value='arrNormalday[cnt].day_id'}-->
                <td><!--{$arrNormalday[cnt].title|h}--></td>
                <td><!--{$arrNormalday[cnt].month|h}-->月<!--{$arrNormalday[cnt].day|h}-->日</td>
                <td class="center">
                    <!--{if $tpl_day_id != $arrNormalday[cnt].day_id}-->
                    <a href="?" onclick="eccube.setModeAndSubmit('pre_edit', 'day_id', <!--{$arrNormalday[cnt].day_id}-->); return false;">編集</a>
                    <!--{else}-->
                    編集中
                    <!--{/if}-->
                </td>
                <td class="center">
                    ...
                    -
                    <!--{else}-->
                    <a href="?" onclick="eccube.setModeAndSubmit('delete', 'day_id', <!--{$arrNormalday[cnt].day_id}-->); return false;">削除</a>
                    <!--{/if}-->
                </td>

...
<div id="products" class="contents-main">
  <form name="form1" id="form1" method="post" action="<!--{$smarty.server.PHP_SELF|escape}-->" enctype="multipart/form-data">
    ...
    <input type="hidden" name="<!--{$smarty.const.TRANSACTION_ID_NAME}-->" value="<!--{$transactionid}-->" />
                    <!--{if $tpl_bag_id != $arrBag[cnt].product_code}-->
                    <table>
                      <tr>
                        <th>ストール・ボレロ商品コード</th>
                        <td>
                      ...
                      <tr>
                        <td>
                          ...
                          <label class="attention">【注意！】削除すると、削除したセット商品のセットのバッグデータが無くなるので、必ず商品登録画面で追加し直してください。</label>
                          <br />
                      ...
                      <tr>
                        <th>画像</th>
                        <td>
                          <img src="<!--{$smarty.const.URL_DIR}-->resize_image.php?image=<!--{$arrForm.main_list_image|escape}-->&width=65&height=65" alt="画像" />
                        </td>
                      </tr>
                    </table>    
                          <div class="btn-area" >
                    ...
                    <table>
                      ...
                      <tr>  
                        ...
                        <th>在庫数</th>
                        <th colspan="2">今週発送数</th>
						<th>予約差止め数</th>
						<th>編集</th>
					  </tr>
					  <!--{section name=cnt loop=$arrBag}-->
					  <tr bgcolor="<!--{if $tpl_bag_id != $arrBag[cnt].product_code}-->#ffffff<!--{else}--><!--{$smarty.const.SELECT_RGB}--><!--{/if}-->">
						<!--{assign var=product_code value=$arrBag[cnt].product_code}-->
                        <!--{*商品コード*}-->
						<td>
                          <!--{$arrBag[cnt].product_code|escape}--><br/>
                        </td>
                        <!--{*画像*}-->
                        <td class="center">
                          <img src="<!--{$smarty.const.URL_DIR}-->resize_image.php?image=<!--{$arrBag[cnt].main_list_image|escape}-->&width=65&height=65" alt="画像" /> 
                        </td>
                        <!--{*使用しているセット商品(個数)*}-->
                        <td>
                          <!--{$arrBag[cnt].set_num|escape}--><label>個</label><br/>
                        </td>
                        <!--{*使用しているセット商品*}-->
                        <td>
                          <!--{section name=key loop=$arrBag[cnt].set_product}-->
                            <!--{$arrBag[cnt].set_product[key]|escape}--><br/>
                          <!--{/section}-->
                        </td>
                        <!--{*在庫数*}-->
                        <td bgcolor="<!--{if $arrBag[cnt].stock != 0}--><!--{if $tpl_bag_id != $arrBag[cnt].product_code}-->#ffffff<!--{else}--><!--{$smarty.const.SELECT_RGB}--><!--{/if}--><!--{else}-->#ff0000<!--{/if}-->">
                          <!--{$arrBag[cnt].stock|escape}--><label>個</label><br/>
                        </td>
                        <!--{*今週発送数*}-->
                        <td bgcolor="<!--{if $arrBag[cnt].shipping_this_week == ""}--><!--{if $tpl_bag_id != $arrBag[cnt].product_code}-->#ffffff<!--{else}--><!--{$smarty.const.SELECT_RGB}--><!--{/if}--><!--{else}-->#f000ff<!--{/if}-->">
                          <label>総数:<!--{$arrBag[cnt].shipping_this_week|default:0}-->件</label>
                        </td>

                        <td bgcolor="<!--{if $arrBag[cnt].shipping_this_week_set == ""}--><!--{if $tpl_bag_id != $arrBag[cnt].product_code}-->#ffffff<!--{else}--><!--{$smarty.const.SELECT_RGB}--><!--{/if}--><!--{else}-->#f000ff<!--{/if}-->">
                          <label>セット:<!--{$arrBag[cnt].shipping_this_week_set|default:0}-->件</label>
                          <!--{section name=key loop=$arrBag[cnt].shipping_product_set}-->
                            <!--{if $arrBag[cnt].shipping_this_week_set != ""}-->
                              <br />(<!--{$arrBag[cnt].shipping_product_set[key]}-->)
                            <!--{/if}-->
                          <!--{/section}-->
                          <br/>
                        </td>
                        <!--{*予約差止め数*}-->
                        <td bgcolor="<!--{if $arrBag[cnt].shipping_chancel == 0}--><!--{if $tpl_bag_id != $arrBag[cnt].product_code}-->#ffffff<!--{else}--><!--{$smarty.const.SELECT_RGB}--><!--{/if}--><!--{else}-->#ff0000<!--{/if}-->">
                          <label><!--{$arrBag[cnt].shipping_chancel|default:0}-->件</label>
                        </td>
                        <!--{*編集*}-->
                        <td>
                          <!--{if $tpl_bag_id != $arrBag[cnt].product_code}-->
                            <a href="<!--{$smarty.server.PHP_SELF|escape}-->" onclick="fnModeSubmit('pre_edit', 'bag_id', '<!--{$arrBag[cnt].product_code}-->'); return false;">編集</a>
                          <!--{else}-->
                          編集中
                          <!--{/if}-->
                        </td>

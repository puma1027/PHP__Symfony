...
<form name="form1" id="form1" method="post" action="?" >
    ...
    <input type="hidden" name="status" value="<!--{if $arrForm.status == ""}-->1<!--{else}--><!--{$arrForm.status|h}--><!--{/if}-->" /><!-- 2020.03.04 SG edit -->
    <input type="hidden" name="search_pageno" value="<!--{$tpl_pageno|h}-->" /><!-- 2020.03.04 SG edit -->
        ...
        <!--{if $tpl_linemax > 0}-->
            ...
            <table class="list">
                ...
                <tr style="background:<!--{$arrORDERSTATUS_COLOR[$status]}-->;">
                    ...
                    <!--{assign var=order_id value="`$arrStatus[cnt].order_id`"}-->
                    <td class="center"><!--{$arrPayment[$order_id]|h}--></td>
...
<!-- RCHJ Change 2012.04.19 : 5->$smarty.const.ORDER_STATUS_DELIV -->
                    <td class="center"><!--{if $arrStatus[cnt].status eq $smarty.const.ORDER_STATUS_DELIV}--><!--{$arrStatus[cnt].commit_date|sfDispDBDate:false}--><!--{else}-->未発送<!--{/if}--></td>
<!-- end -->
                </tr>
                <!--{/section}-->
            </table>

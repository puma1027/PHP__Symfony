...
require_once MODULE_REALDIR . 'mdl_remise/class_ac_Ex/SC_Fpdf_AC_Ex.php';
 ...
 */
class LC_Page_Admin_Order_Pdf_AC extends LC_Page_Admin_Ex
{
    /**
     * Page を初期化する.
     *
    ...
    {
        parent::init();
        $this->tpl_mainpage = MDL_REMISE_AC_TEMPLATE_PATH . 'remise_pdf_AC_input.tpl';
    ...
    function createFromValues($order_id, $pdf_order_id)
    {
        ...
        $arrForm['msg3'] = 'ご確認をお願いいたします。';

        // 定期購買に関するメッセージ
        $arrForm['etc1'] = 'こちらの商品は定期購買商品です。';
        $arrForm['etc3'] = '予めご了解いただきますよう、お願いいたします。';

        // 注文番号があったら、セットする
        if (SC_Utils_Ex::sfIsInt($order_id)) {
            $arrForm['order_id'][0] = $order_id;
            $arrOrder = $objPurchase->getOrder($order_id);
            $arrForm['etc2_msg'] = '次回の課金日は' . $arrOrder['plg_remiseautocharge_next_date'] . 'です。';
            $arrForm['etc2_data'] = $arrOrder['plg_remiseautocharge_next_date'];
        }
        else if (is_array($pdf_order_id)) {
            sort($pdf_order_id);
            $arrForm['etc2_msg'] = '次回の課金日は[○年○月○日]です。';
            foreach ($pdf_order_id AS $key => $val) {
                $arrForm['order_id'][] = $val;
                $arrOrder = $objPurchase->getOrder($val);
                $arrForm['etc2_data'][$val] = $arrOrder['plg_remiseautocharge_next_date'];
            }
        }
    ...
    function createPdf(&$objFormParam)
    {
        ...
        // エラー入力なし
        if (count($arrErr) == 0) {
            $objFpdfAC = new SC_Fpdf_AC_Ex($arrRet['download'], $arrRet['title']);
            foreach ($arrRet['order_id'] AS $key => $val) {
                $arrPdfData = $arrRet;
                $arrPdfData['order_id'] = $val;
                $arrPdfData['etc2'] = str_replace('[○年○月○日]', $arrRet['etc2_data'][$val], $arrRet['etc2_msg']);
                $objFpdfAC->setData($arrPdfData);
            }
            $objFpdfAC->createPdf();
            return true;
        } else {
    ...
    function lfInitParam(&$objFormParam)
    {
        ...
        $objFormParam->addParam('備考2行目（メッセージ）',  'etc2_msg',     STEXT_LEN,      'KVa',  array('MAX_LENGTH_CHECK'));
        $objFormParam->addParam('備考2行目（データ）',      'etc2_data',    STEXT_LEN,      'KVa',  array('MAX_LENGTH_CHECK'));

...
class SC_Fpdf_AC extends SC_Helper_FPDI
{
    ...
    function setOrderData()
    {
        ...
        $this->Cell(67, 8, number_format($this->arrDisp['plg_remiseautocharge_total']) . ' 円', 0, 2, 'R', 0, '');
        ...
        for ($i = 0; $i < count($this->arrDisp['quantity']); $i++) {
            ...
            // 税込金額（単価）
            $data[1] = $this->arrDisp['plg_remiseautocharge_total'] - $this->arrDisp['deliv_fee'];
        ...
        $arrOrder[$i][1] = '';
        $arrOrder[$i][2] = '商品合計';
        $arrOrder[$i][3] = number_format($this->arrDisp['plg_remiseautocharge_total'] - $this->arrDisp['deliv_fee']) . $monetary_unit;
        ...
        $arrOrder[$i][1] = '';
        $arrOrder[$i][2] = '請求金額';
        $arrOrder[$i][3] = number_format($this->arrDisp['plg_remiseautocharge_total']) . $monetary_unit;
    ...
    // 商品詳細データの取得
    function lfGetProductDetail($order_id)
    {
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $col = 'product_id, product_class_id, product_code, product_name, classcategory_name1, classcategory_name2, price, quantity, point_rate';
        $where = 'order_id = ?';
        $objQuery->setOrder('order_detail_id');
        $data = $objQuery->select($col, 'dtb_order_detail', $where, array($order_id));
        $arrRet = $objQuery->select($col, 'dtb_products', $where, array($data['product_id']));
        return $arrRet;
    }
}

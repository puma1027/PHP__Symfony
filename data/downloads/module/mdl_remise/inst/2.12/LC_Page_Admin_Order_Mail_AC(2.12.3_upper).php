 ...
 */
class LC_Page_Admin_Order_Mail_AC extends LC_Page_Admin_Order_Ex
{
    /**
     * Page を初期化する.
     *
    ...
    {
        parent::init();
        $this->tpl_mainpage = 'order/mail_ac.tpl';
    ...
    function action()
    {
        ...
        if (preg_match("/^[0-9|\,]*$/", $post['order_id_array'])) {
            ...
            // エラーで元に戻す
            SC_Response_Ex::sendRedirect(ADMIN_ORDER_URLPATH . 'remise_ac_order.php');
    ...
    function confirm(&$objFormParam)
    {
        ...
        if (count($arrErr) == 0) {
            ...
            $this->tpl_mainpage = 'order/mail_ac_confirm.tpl';
            return true;
        }
        return $arrErr;
    }
    ...
    function changeData(&$objFormParam)
    {
        ...
        // 有効選択時
        else if (SC_Utils_Ex::sfIsInt($template_id)) {
            $where = 'template_id = ?';

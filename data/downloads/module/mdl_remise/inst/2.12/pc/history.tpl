...
<div id="mypagecolumn">
    ...
    <div id="mycontents_area">
        ...
        <div class="mycondition_area clearfix">
            ...
            <!--{if $tpl_arrOrderDetail[0].product_type_id == $smarty.const.PRODUCT_TYPE_AC_REMISE || $tpl_arrOrderDetail[0].product_type_id == $smarty.const.PRODUCT_TYPE_AC_REMISE_DL}-->
                <!--{if $tpl_arrOrderData.memo10 == "autocharge"}-->
                    <div>
                        <form action="../user_data/remise_ac_refusal.php" method="post">
                            <input type="hidden" name="<!--{$smarty.const.TRANSACTION_ID_NAME}-->" value="<!--{$transactionid}-->" />
                            <div class="btn">
                                <input type="hidden" name="mode" value="quit">
                                <input type="hidden" name="order_id" value="<!--{$tpl_arrOrderData.order_id|h}-->">
                                <input type="image" onmouseover="chgImg('<!--{$smarty.const.ROOT_URLPATH}-->user_data/mdl_remise/img/btn_acorder_quit_on.jpg', this);" onmouseout="chgImg('<!--{$smarty.const.ROOT_URLPATH}-->user_data/mdl_remise/img/btn_acorder_quit.jpg', this);" src="<!--{$smarty.const.ROOT_URLPATH}-->user_data/mdl_remise/img/btn_acorder_quit.jpg" alt="会員退会を行う" name="submit" value="退会を行う" />
                            </div>
                        </form>
                    </div>
                    <div>
                        <form action="../user_data/remise_ac_update.php" method="post">
                            <input type="hidden" name="<!--{$smarty.const.TRANSACTION_ID_NAME}-->" value="<!--{$transactionid}-->" />
                            <div class="btn" style="margin-right:30px;">
                                <input type="hidden" name="mode" value="update">
                                <input type="hidden" name="order_id" value="<!--{$tpl_arrOrderData.order_id|h}-->">
                                <input type="image" onmouseover="chgImg('<!--{$smarty.const.ROOT_URLPATH}-->user_data/mdl_remise/img/btn_acorder_update_on.jpg', this);" onmouseout="chgImg('<!--{$smarty.const.ROOT_URLPATH}-->user_data/mdl_remise/img/btn_acorder_update.jpg', this);" src="<!--{$smarty.const.ROOT_URLPATH}-->user_data/mdl_remise/img/btn_acorder_update.jpg" alt="会員情報の更新を行う" name="submit" value="会員情報の更新を行う" />
                            </div>
                        </form>
                    </div>
                <!--{elseif $tpl_arrOrderData.memo10 == "continue"}-->
                    <div style="float:right;">
                        ※このご注文は、定期購買でございます。<br />
                        次回の課金日は、<!--{$tpl_arrOrderData.plg_remiseautocharge_next_date}-->となります。
                    </div>
                <!--{else}-->
                    <div style="float:right;">
                        <!--{if $tpl_arrOrderDetail[0].status == $smarty.const.ORDER_PENDING }-->
                            ※このご注文は、決済が完了しておりません。
                        <!--{else}-->
                            ※このご注文は、既に退会処理済みです。
                        <!--{/if}-->
        ...
        <table summary="購入商品詳細">
            ...
            <tr>
                ...
                <td class="alignC">
                    <!--{if $orderDetail.product_type_id == $smarty.const.PRODUCT_TYPE_DOWNLOAD || $tpl_arrOrderDetail[0].product_type_id == $smarty.const.PRODUCT_TYPE_AC_REMISE_DL}-->
        ...
        <!--{if $tpl_arrOrderDetail[0].product_type_id == $smarty.const.PRODUCT_TYPE_AC_REMISE || $tpl_arrOrderDetail[0].product_type_id == $smarty.const.PRODUCT_TYPE_AC_REMISE_DL}-->
            <h3>定期課金情報</h3>
                <table summary="定期課金情報" class="acordername">
                    <colgroup width="30%"></colgroup>
                    <colgroup width="70%"></colgroup>
                    <tr>
                        <th class="alignL">定期課金　メンバーID</th>
                        <td><!--{$tpl_arrOrderData.plg_remiseautocharge_member_id}--></td>
                    </tr>
                    <tr>
                        <th class="alignL">定期課金  金額</th>
                        <td><span class="price"><!--{$tpl_arrOrderData.plg_remiseautocharge_total|number_format}-->円</span></td>
                    </tr>
                    <tr>
                        <th class="alignL">定期課金　課金開始日</th>
                        <td><!--{$tpl_arrOrderData.plg_remiseautocharge_next_date}--></td>
                    </tr>
                    <tr>
                        <th class="alignL">定期課金　課金間隔</th>
                        <td><!--{$tpl_arrOrderData.plg_remiseautocharge_interval}-->ヶ月</td>
                    </tr>
                </tbody>
            </table>
        <!--{/if}-->

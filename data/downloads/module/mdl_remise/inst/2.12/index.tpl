...
<div id="order" class="contents-main">
    <form name="search_form" id="search_form" method="post" action="?">
        ...
        <input type="hidden" name="credit_order_id" value="" />
        ...
        <table>
            ...
            <!--{* remise決済モジュール用 *}-->
            <!--{assign var=path value=`$smarty.const.MODULE_REALDIR`mdl_remise/tmp/order_search.tpl}-->
            <!--{if file_exists($path) && $CreditCheckVisible == 'yes'}-->
                <!--{include file=$path}-->
            <!--{else}-->
            <tr>
                <th>支払方法</th>
    ...
    <!--{if count($arrErr) == 0 and ($smarty.post.mode == 'search' or $smarty.post.mode == 'delete' or $smarty.post.mode == 'credit_sales')}-->
        ...
        <form name="form1" id="form1" method="post" action="?">
            ...
            <div class="btn">
                ...
                <!--{* remise決済モジュール用 *}-->
                <!--{assign var=path value=`$smarty.const.MODULE_REALDIR`mdl_remise/tmp/order_search_result_salessubmit.tpl}-->
                <!--{if file_exists($path) && $CreditCheckVisible == 'yes'}-->
                    <!--{include file=$path}-->
                <!--{/if}-->
            ...
            <!--{if count($arrResults) > 0}-->
                ...
                <table class="list">
                    <!--{* remise決済モジュール用 *}-->
                    <!--{assign var=path value=`$smarty.const.MODULE_REALDIR`mdl_remise/tmp/order_search_result(2.12).tpl}-->
                    <!--{if file_exists($path)}-->
                        <!--{include file=$path}-->
                    <!--{else}-->
                        ...
                        <tr>
                            ...
                            <th>編集</th>
                            <th>
                                メール <input type="checkbox" name="mail_check" id="mail_check" onclick="fnAllCheck(this, 'input[name=mail_order_id[]]')" />
                            </th>
                            <th>削除</th>
                        </tr>
                        ...
                        <tr style="background:" <!--{$arrORDERSTATUS_COLOR[$status]}-->;">
                            <td class="center">
                                <!--{$arrResults[cnt].create_date|sfDispDBDate}-->
                            </td>
                            <td class="center">
                                <!--{$arrResults[cnt].order_id}-->
                            </td>
                            <td>
                                <!--{$arrResults[cnt].order_name01|h}-->
                                <!--{$arrResults[cnt].order_name02|h}-->
                            </td>
                            <!--{assign var=payment_id value="`$arrResults[cnt].payment_id`"}-->
                            <td class="center">
                                <!--{$arrPayments[$payment_id]}-->
                            </td>
                            <!--{if $CreditCheckVisible == 'yes'}-->
                            <td class="center">
                            ...
                            <!--{/if}-->
                            <td class="right">
                                <!--{$arrResults[cnt].payment_total|number_format}-->
                            </td>
                            <td class="center">
                                <!--{$arrResults[cnt].commit_date|sfDispDBDate|default:"未発送"}-->
                            </td>
                            <td class="center">

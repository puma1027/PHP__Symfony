...
class LC_Page_Shopping_Ex extends LC_Page_Shopping
{
    ...
    function action()
    {
        ...
        if ($_SESSION["twoclick"] != "4" && $_SESSION["twoclick"] != "3") {
        ...
        switch ($this->getMode()) {
            ...
            case 'nonmember_confirm':
                ...
                $objFormParam->setParam($_POST);
                $this->arrErr = parent::lfCheckError($objFormParam);
    ...
    {
        $prev_url = $_SESSION['cart_referer_url'];
        $parts = substr($prev_url, 0, 5);
        if ($parts === "https") {
            $pattern = HTTPS_URL;
        } else {
            $pattern = HTTP_URL;
        }
        $prev_page = str_replace($pattern, "", $prev_url);
        if ($_SESSION["twoclick"] === "3") {
            unset($_SESSION["twoclick"]);   // ログインしてきたら、セッションを消去
            return '../cart/index.php';     // カートからの場合はカートへ戻す
        }
        else if ($_SESSION["twoclick"] === "4") {
            unset($_SESSION["twoclick"]);
            return '../' . $prev_page;  // 商品詳細からの場合、商品詳細へ戻す
        }

        switch ($product_type_id) {
            case PRODUCT_TYPE_DOWNLOAD:
            case PRODUCT_TYPE_AC_REMISE_DL:
                $objPurchase->saveOrderTemp($uniqid, array(), $objCustomer);
                $objSiteSess->setRegistFlag();
                return 'payment.php';

            case PRODUCT_TYPE_NORMAL:
            case PRODUCT_TYPE_AC_REMISE:
            default:
                return 'deliv.php';
        }
    }
    ...
    function setFormParams(&$objFormParam, &$objPurchase, $uniqid)
    {
        ...
        // count($arrShippingTemp) > 1 は複数配送であり、$arrShippingTemp[0] は注文者が格納されている

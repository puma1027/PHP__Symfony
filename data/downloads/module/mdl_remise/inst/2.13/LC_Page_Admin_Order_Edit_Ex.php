...
class LC_Page_Admin_Order_Edit_Ex extends LC_Page_Admin_Order_Edit
{
    ...
    function lfInitParam(&$objFormParam)
    {
        ...
        $objFormParam->addParam("del_flg",              "del_flg");             // 削除フラグ（復活用）
    }
    ...
    function doRegister($order_id, &$objPurchase, &$objFormParam, &$message, &$arrValuesBefore)
    {
        ...
        if ($objFormParam->arrValue['del_flg'] == '1' && $objFormParam->arrValue['status'] != ORDER_CANCEL && $objFormParam->arrValue['status'] != ORDER_PENDING) {
            $objFormParam->setValue('del_flg', '0');
        }

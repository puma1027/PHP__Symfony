    ...
    <form name="search_form" id="search_form" method="post" action="?">
        ...
        <table>
            ...
            <tr>
                <th>全登録データ確認</th>
                <td colspan="3">
                    <input type="checkbox" name="all_disp" id="all_disp" />
                    <label for="all_disp">削除済みのデータを表示する</label>
                </td>
            </tr>
        </table>
    ...
    <!--{if count($arrErr) == 0 and ($smarty.post.mode == 'search' or $smarty.post.mode == 'delete'  or $smarty.post.mode == 'credit_sales')}-->
        ...
        <form name="form1" id="form1" method="post" action="?">
            ...
            <!--{if count($arrResults) > 0}-->
                ...
                <table class="list">
                    <!--{* remise決済モジュール用 *}-->
                    <!--{assign var=path value=`$smarty.const.MODULE_REALDIR`mdl_remise/tmp/order_search_result(2.13).tpl}-->
                    <!--{if file_exists($path)}-->
                        <!--{include file=$path}-->
                    <!--{else}-->

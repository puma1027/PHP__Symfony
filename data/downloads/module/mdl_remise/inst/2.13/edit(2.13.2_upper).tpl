...
<form name="form1" id="form1" method="post" action="?">
    ...
    <div id="order" class="contents-main">
        ...
        <!--{foreach name=shipping from=$arrAllShipping item=arrShipping key=shipping_index}-->
            ...
            <!--{if $tpl_shipping_quantity > 1}-->
                <h2>
                    届け先商品情報&nbsp;<a class="btn-normal" href="javascript:;" name="add_product" onclick="eccube.openWindow('<!--{$smarty.const.ROOT_URLPATH}--><!--{$smarty.const.ADMIN_DIR}-->order/product_select.php?order_id=<!--{$arrForm.order_id.value|h}-->&shipping_id=<!--{$shipping_index}-->', 'search', '615', '500', {menubar:'no'}); return false;">商品の追加</a>
                </h2>
        ...
        <table class="form">
            ...
            <!--{* remise決済モジュール用 *}-->
            <!--{assign var=path value=`$smarty.const.MODULE_REALDIR`mdl_remise/tmp/order_edit.tpl}-->
            <!--{if file_exists($path) && $arrForm.payment_module.value == "mdl_remise"}-->
                <!--{include file=$path}-->
            <!--{/if}-->
            ...
            <!--{/if}-->
            <!--{if $arrForm.del_flg.value == "1"}-->
            <tr>
                <th>削除済みデータの復旧について</th>
                <td>
                    対応状況のステータス変更を行うと、削除済みのデータを受注一覧へ復旧させることが出来ます。<br />
                    <span class="attention">
                        ※ 「キャンセル」及び「決済処理中」に変更する場合、復旧することができません。<br />
                        ※ キャンセル分の在庫は手動で変更してください。
                    </span>
                </td>
            </tr>
            <!--{/if}-->
